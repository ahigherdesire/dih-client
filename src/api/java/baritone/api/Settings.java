...
public final class Settings {
    ...
    /** Ticks {@code #menu} waits after clicking a menu slot before scanning for the next one. */
    ...
    /**
     * Case-insensitive word to watch for in incoming chat while {@code #listentomine}
     * is enabled. On a match: run {@link #listenToMineWarp}, free an inventory slot,
     * then mine {@link #listenToMineTarget}.
     */
    public final Setting<String> listenToMineWord = new Setting<>("legendary");

    /** Command run when the trigger word is seen. {@code /x} = server command, {@code #x} = Baritone command. Default {@code /warp mine}. */
    public final Setting<String> listenToMineWarp = new Setting<>("/warp mine");

    /** Block(s) to mine after warping. Passed to {@code #mine}. Default {@code ancient_debris}. */
    public final Setting<String> listenToMineTarget = new Setting<>("ancient_debris");

    /** Ticks to wait after the warp before clearing a slot and mining, so the teleport lands first. Default 40 (2s). */
    public final Setting<Integer> listenToMineWarpDelay = new Setting<>(40);

    /** Ticks to ignore further trigger words after one fires, so a burst of chat doesn't re-warp. Default 200 (10s). */
    public final Setting<Integer> listenToMineCooldown = new Setting<>(200);
    ...
    public final Setting<Consumer<Component>> logger = new Setting<>((msg) -> {
        try {
            // MC 26.2: Gui.getChat() was removed and chat is now keyed by ChatMethod with no public
            // ChatComponent accessor, so route log output through the ChatListener's system-message
            // handler (which adds it to the chat HUD). The custom Baritone GuiMessageTag icon is no
            // longer attachable without private Gui access, so useMessageTag no longer tags log lines.
