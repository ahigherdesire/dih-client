...
public final class GameEventHandler implements IEventBus, Helper {
    ...
    /** Listener classes whose render failure has already been logged (log once, keep rendering the rest). */
    private static final java.util.Set<Class<?>> RENDER_FAILURES_LOGGED = java.util.concurrent.ConcurrentHashMap.newKeySet();

    @Override
    public final void onRenderPass(RenderEvent event) {
        // Runs inside LevelRenderer.render: one broken overlay must not crash the frame or hide the others.
        for (IGameEventListener l : listeners) {
            try {
                l.onRenderPass(event);
            } catch (Throwable t) {
                if (RENDER_FAILURES_LOGGED.add(l.getClass())) {
                    org.slf4j.LoggerFactory.getLogger("Baritone/Render")
                        .warn("Render pass of {} failed; continuing without it this frame", l.getClass().getName(), t);
                }
            }
        }
    }
