...
import dihclient.gui.macro.editor.ActionFieldSchema;
...
public final class MacroActionRegistry {
    ...
    public static AddonRegistrationResult registerDetailed(MacroActionEntry entry) {
        ...
        if (typeId == null || !typeId.contains(":")) {
            return reject(ownerAddonId, "macro action", typeId, "non-namespaced type id - scope it with DihAddons.id(...) so it becomes addonId:localId");
        }
        if (!typeId.startsWith(ownerAddonId + ":")) {
            return reject(ownerAddonId, "macro action", typeId, "foreign namespace - the id must start with your addon id; scope it with DihAddons.id(...)");
        }
        ...
        try {
            ...
            DihClientAddon.LOG.warn("[MacroActions] Rejecting action '{}' because its factory threw during validation", typeId, t);
    ...
    private static AddonRegistrationResult reject(String ownerAddonId, String kind, String id, String reason) {
        DihClientAddon.LOG.warn("[MacroActions] Rejecting {} '{}': {}", kind, id, reason);
    ...
    private static String validateProbe(String typeId, MacroActionEntry entry, MacroAction probe, ActionFieldSchema schema) {
        ...
        try {
            ...
            DihClientAddon.LOG.warn("[MacroActions] Rejecting '{}' because toTag() threw", typeId, t);
        ...
        if (schema != null) {
            for (dihclient.gui.macro.editor.FieldDef field : schema.fields()) {
                String key = field.key();
                if (field.type() == dihclient.gui.macro.editor.FieldType.BLOCK_POS) {
