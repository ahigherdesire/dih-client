...
public final class HudElements {
    ...
    public static AddonRegistrationResult registerDetailed(HudElementProvider provider) {
        ...
        if (id == null || !id.contains(":")) {
            return reject(ownerAddonId, id, "non-namespaced id - scope it with DihAddons.id(...) so it becomes addonId:localId");
        }
        if (!id.startsWith(ownerAddonId + ":")) {
            return reject(ownerAddonId, id, "foreign namespace - the id must start with your addon id; scope it with DihAddons.id(...)");
        }
    ...
    private static AddonRegistrationResult validateSize(String ownerAddonId, String id, HudElementProvider provider) {
        ...
        try {
            ...
            DihClientAddon.LOG.warn("[Hud] Rejecting HUD element '{}': sizing threw", id, t);
    ...
    private static AddonRegistrationResult reject(String ownerAddonId, String id, String reason) {
        DihClientAddon.LOG.warn("[Hud] Rejecting HUD element '{}': {}", id, reason);
