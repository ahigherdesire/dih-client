...
public class DelayCommand extends Command {
    ...
    public void build(LiteralArgumentBuilder<DihCommandSource> root) {
        ...
        root.then(RequiredArgumentBuilder.<DihCommandSource, String>argument("state", StringArgumentType.word())
    ...
    private static int apply(String state) {
        boolean current = DihSharedState.get().shouldDelayGuiPackets();
        if (state != null) {
            ...
            if (!lower.equals("on") && !lower.equals("enable") && !lower.equals("true") && !lower.equals("1")
                    ...
                    && !lower.equals("toggle")) {
                DihClientMessaging.sendPrefixed("Unknown delay state: " + state);
                DihClientMessaging.sendPrefixed("Use on, off, or toggle.");
                return SUCCESS;
            }
        }
