...
public class DihProtectorHttpUtilMixin {
    ...
    private static InputStream dih$blockLocal(
        ...
        if (!DihProtector.shouldBlockLocalUrls()) return original.call(instance);
    ...
    private static void rejectLocal(URL url) {
        ...
        if (DihProtectorLocalAddressUtil.shouldBlock(host)) {
            throw new IllegalStateException("[DihProtector] Refused resource pack download from local address: " + url);
        }
    }
