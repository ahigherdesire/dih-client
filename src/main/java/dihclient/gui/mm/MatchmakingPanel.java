...
import dihclient.util.DihSharePickerOverlay;
...
import dihclient.util.mm.Lobby;
import dihclient.util.mm.LobbyListing;
import dihclient.util.mm.LobbySettings;
import dihclient.util.mm.MatchmakingManager;
import dihclient.util.mm.MmBlobs;
import dihclient.util.mm.MmCardActions;
import dihclient.util.mm.MmText;
import dihclient.util.mm.MmChatLine;
import dihclient.util.mm.MmPeer;
import dihclient.util.mm.MmPrefs;
import dihclient.util.mm.MmShare;
...
public final class MatchmakingPanel {
    ...
    private DihPacketInspectOverlay inspectOverlay;
    private DihGuiViewOverlay guiViewOverlay;
    private DihFilterViewOverlay filterViewOverlay;
    private DihModuleViewOverlay moduleViewOverlay;
    private DihSharePickerOverlay sharePicker;

    private static final int ICON_CACHE_MAX = 64;
    private final Map<String, ItemStack> itemIconCache = new LinkedHashMap<>();

    private DihTheme.State cachedThemeState;
    ...
    public void render(GuiGraphicsExtractor g, int x, int y, int w, int h, int mouseX, int mouseY, float partial) {
        ...
        if (!dihclient.util.DihDiscordLogin.hasSession()) { renderSignInGate(g, x, y, w, h, mouseX, mouseY); return; }
    ...
    private void renderSignInGate(GuiGraphicsExtractor g, int x, int y, int w, int h, int mx, int my) {
        ...
        ty += 18;
        String notice = dihclient.util.DihDiscordLogin.authGateNotice();
        boolean gated = dihclient.util.DihDiscordLogin.isGateCode(notice);
        String[] lines;
        if (gated) {
            ...
            lines = new String[]{
                ...
                "",
                "Grab the latest from dihclient.com,",
        ...
        if (gated) {
            button(g, bxc, ty, bw2, 18, "Get the latest version", ACCENT, TEXT, mx, my,
                () -> DihLinks.open(DihLinks.WEBSITE));
            ty += 24;
        }
        button(g, bxc, ty, bw2, 18, "Sign in with Discord", gated ? BORDER : ACCENT, TEXT, mx, my, this::signInDiscord);
        ty += 24;
        button(g, bxc, ty, bw2, 16, "DIH Client website", BORDER, TEXT, mx, my, () -> DihLinks.open(DihLinks.WEBSITE));
    }
    ...
    private void renderConnectingGate(GuiGraphicsExtractor g, int x, int y, int w, int h, int mx, int my) {
        ...
        if (mm.connState() == MatchmakingManager.ConnState.OFFLINE) {
            if (DihDiscordLogin.isGateCode(DihDiscordLogin.authGateNotice())) {

                drawText(g, DihDiscordLogin.errorMessage(DihDiscordLogin.authGateNotice()), cx, ty, ERROR, true, w - 24); ty += 13;
                String min = DihDiscordLogin.requiredMinVersion();
                if (!min.isEmpty()) { drawText(g, "Minimum version: " + min, cx, ty, MUTED, true, w - 24); ty += 11; }
                drawText(g, "Get the latest download at dihclient.com", cx, ty, MUTED, true, w - 24); ty += 11;
            } else {
    ...
    private int renderLobbyServerBar(GuiGraphicsExtractor g, int x, int y, int w, int mx, int my) {
        ...
        dihclient.gui.vanillaui.components.Card.render(
            dihclient.gui.vanillaui.UiContexts.overlay(g, font, mx, my), UiBounds.of(x + 8, y, w - 16, rowH));
    ...
    private int renderBrowse(GuiGraphicsExtractor g, int x, int y, int w, int top, int bottom, int mx, int my) {
        ...
        for (LobbyListing l : listings) {
            ...
            if (visible) {
                ...
                dihclient.gui.vanillaui.components.Card.render(
                    dihclient.gui.vanillaui.UiContexts.overlay(g, font, mx, my), UiBounds.of(x, y, w, 30));
    ...
    private void doCreate() {
        ...
        if (name.isEmpty()) { DihNotifications.show("Enter a lobby name.", WARN); return; }
        int max = parseMax(maxPlayers.text());
        int capLimit = capLimit();
        if (max <= 0) max = capLimit;
        if (max > capLimit) {
            max = capLimit;
            DihNotifications.show("Max players is capped at " + capLimit + ".", WARN);
        }
        if (createPublic) {
            if (prefs.myDupeStatus() < 0) { DihNotifications.show("Pick a dupe status (needs or has).", WARN); return; }
    ...
    private void doJoinPublic() {
        ...
        if (code.isEmpty()) { DihNotifications.show("Enter a lobby code.", WARN); return; }
        mm.joinPublic(code); afterJoin();
    }

    private void doJoinPrivate() {
        String key = passphrase.text().trim();
        if (key.isEmpty()) { DihNotifications.show("Paste the room key.", WARN); return; }
    ...
    private static int capLimit() {
        return switch (DihDiscordLogin.role()) {
    ...
    private void renderShareToolbar(GuiGraphicsExtractor g, int x, int r1, int w, int mx, int my) {
        ...
        int colW = (w - gap * 4) / 5;
        int r2 = r1 + 18;
        int qn = DihSharedState.get().getDelayedPackets().size();
        boolean hasMacros = !DihMacroManager.get().getAll().isEmpty();
    ...
    private void toggleModulePicker() {
        ...
        sharePicker.open("Share Module", "Search modules...", filter -> {
            ...
            for (Module m : mods) {
                ...
                if (!cat.equals(lastCat)) { out.add(DihSharePickerOverlay.Row.header(cat)); lastCat = cat; }
                Module module = m;
                out.add(DihSharePickerOverlay.Row.item(m.name(), null, () -> shareModule(module)));
            }
            return out;
        });
    }
    ...
    private static String categoryLabel(Module m) {
        dihclient.modules.ModuleCategory c = m.category();
    ...
    private void shareGui() {
        ...
        if (b == null) { DihNotifications.show("Open a container to share its GUI.", WARN); return; }
        mm.offerBlob(b);
    }
    private void shareItem() {
        MmMessages.BlobOffer b = MmBlobs.captureHeldItem();
        if (b == null) { DihNotifications.show("Hold an item to share it.", WARN); return; }
        mm.offerBlob(b);
    }
    ...
    private void shareServer() {
        ...
        if (b == null) { DihNotifications.show("Join a server first.", WARN); return; }
        mm.offerBlob(b);
    }
    private void clearChat() { mm.clearChat(); }
    private void signInDiscord() {
        DihNotifications.show("Opening Discord sign-in in your browser…", 0xFF35D873);
        dihclient.util.DihDiscordLogin.signIn(err -> {
            if (err.isEmpty()) {
                mm.openDirectory();
                DihNotifications.show("Signed in to matchmaking.", 0xFF35D873);
            }

            else if (!dihclient.util.DihDiscordLogin.isGateCode(err))
                DihNotifications.show(dihclient.util.DihDiscordLogin.errorMessage(err), 0xFFFF5B5B);
        });
    }
    ...
    private void toggleKillSwitch() {
        ...
        DihNotifications.show(on ? "Kill switch on. Sharing off." : "Kill switch off.", on ? 0xFFFFC857 : 0xFF35D873);
    }

    private void signOutDiscord() {

        mm.leave();
        dihclient.util.DihDiscordLogin.signOut();
        DihNotifications.show("Signed out of matchmaking.", 0xFFFFC857);
    }

    private void shareTpaInvite() {
        String name = mm.selfTpaName();
        if (name == null || name.isBlank()) { DihNotifications.show("Can't read your username.", WARN); return; }
    ...
    private void renderCard(GuiGraphicsExtractor g, MmChatLine line, Map<String, String> names, int x, int y, int w, int mx, int my) {
        ...
        dihclient.gui.vanillaui.components.Card.render(
            dihclient.gui.vanillaui.UiContexts.overlay(g, font, mx, my), UiBounds.of(x, y, w, 28));
        ...
        switch (line.kind) {
            case MACRO_CARD -> cardBtn(g, rightX, y, "Inspect", INFO, mx, my,
                () -> openMacroEditor(DihClipboardHelper.deserializeMacroFromBase64(line.macro.hash)));
            case COMMAND_CARD -> {
                if (tpa) {
        ...
        switch (b.kind) {
            ...
            case "filter" -> {
                ...
                cardBtn(g, rightX, y, "Import", ACCENT, mx, my, () -> {
                    ...
                    DihNotifications.show("Imported " + n + " filtered packet(s).", ACCENT);
                });
            }
            case "server" -> cardBtn(g, rightX, y, "Join", ACCENT, mx, my, () -> joinSharedServer(b));
            case "steps" -> {
                rightX = cardBtn(g, rightX, y, "To editor", INFO, mx, my,
                    () -> openMacroEditor(DihClipboardHelper.deserializeMacroFromBase64(b.data)));
    ...
    private void doSend() {
        if (!mm.inLobby()) { DihNotifications.show("Join a lobby first.", WARN); return; }
        String text = chatInput.text().trim();
        if (text.isEmpty()) return;

        String type = text.length() >= 24 ? DihClipboardHelper.detectShareType(text) : null;
        if ("dih_macro_steps".equals(type)) {

            MmMessages.BlobOffer steps = buildStepsOffer(text);
            if (steps != null) { mm.offerBlob(steps); chatInput.setText(""); return; }
        } else if (type != null && type.startsWith("dih_macro")) {
            DihMacro macro = DihClipboardHelper.deserializeMacroFromBase64(text);
            ...
            List<DihSharedState.QueuedPacket> q = DihClipboardHelper.deserializeQueueFromBase64(text);
        ...
        if (text.startsWith("/") || DihCommands.isDihCommandMessage(text)) {
    ...
    private void shareQueue() {
        List<DihSharedState.QueuedPacket> q = DihSharedState.get().getDelayedPackets();
        MmMessages.PacketOffer offer = MmShare.buildPacketOffer(q, q.size() + (q.size() == 1 ? " packet" : " packets"));
        if (offer == null) { DihNotifications.show("Your packet queue is empty.", WARN); return; }
        mm.offerPacket(offer);
    }

    private void shareMacro(DihMacro macro) {
        MmMessages.MacroOffer offer = MmShare.buildMacroOffer(macro);
        if (offer != null) mm.offerMacro(offer);
    }

    private MmMessages.BlobOffer buildStepsOffer(String stepsHash) {
        DihMacro macro = DihClipboardHelper.deserializeMacroFromBase64(stepsHash);
    ...
    private void shareModule(Module m) {
        ...
        if (b == null) { DihNotifications.show("Could not capture module settings.", ERROR); return; }
        mm.offerBlob(b);
    }
    ...
    private void openModuleView(MmMessages.BlobOffer b) {
        if (moduleViewOverlay == null) moduleViewOverlay = new DihModuleViewOverlay(font);
    ...
    private void applyModuleBlob(MmMessages.BlobOffer b) {
        ...
        if (n < 0) { DihNotifications.show("Can't apply. Bad data.", ERROR); return; }
        DihNotifications.show("Applied " + n + " setting(s) to " + MmBlobs.moduleName(b) + ".", ACCENT);
    }

    private void addToQueue(MmMessages.PacketOffer offer) {
        int n = MmShare.addToQueue(offer);
        if (n < 0) { DihNotifications.show("Could not read shared queue.", ERROR); return; }
        DihNotifications.show("Added " + n + " packet(s) to your queue.", ACCENT);
    }
    ...
    private int renderMembersList(GuiGraphicsExtractor g, int x, int top, int w, int bottom, int mx, int my) {
        ...
        if (!code.isBlank()) {
            ...
            dihclient.gui.vanillaui.components.Card.render(
                dihclient.gui.vanillaui.UiContexts.overlay(g, font, mx, my), UiBounds.of(x, y, w, 20));
            ...
            button(g, x + w - 54, y + 2, 48, 16, "Copy", ACCENT, TEXT, mx, my,
                () -> { copyToClipboard(code); DihNotifications.copied(pub ? "Room code copied." : "Room key copied."); });
            y += 24;
        }
        ...
        dihclient.gui.vanillaui.components.Card.render(
            dihclient.gui.vanillaui.UiContexts.overlay(g, font, mx, my), UiBounds.of(x, y, w, selfH));
        String selfRole = lb != null && lb.announcement
            ? (DihDiscordLogin.isAdmin() ? "  [Admin]" : mm.canSend() ? "  [Speaker]" : "") : "";
        ...
        for (MmPeer p : peers) {
            ...
            if (visible) {
                ...
                dihclient.gui.vanillaui.components.Card.render(
                    dihclient.gui.vanillaui.UiContexts.overlay(g, font, mx, my), UiBounds.of(x, y, w, rowH));
    ...
    private void renderSettings(GuiGraphicsExtractor g, DirectRenderContext ctx, int x, int top, int w, int bottom, int mx, int my) {
        ...
        try {
            ...
            dihclient.gui.vanillaui.components.Card.render(
                dihclient.gui.vanillaui.UiContexts.overlay(g, font, mx, my), UiBounds.of(x, y, w, 18));
    ...
    private void openInspector(DihPacketLoggerOverlay.LogEntry entry) {
        if (entry == null) { DihNotifications.show("Could not rebuild packet to inspect.", ERROR); return; }
        if (inspectOverlay == null) inspectOverlay = new DihPacketInspectOverlay(font);
        inspectOverlay.open(entry, 60, 60);
        presentOverlay(inspectOverlay, true);
    }

    private void openItemInspect(ItemStack stack) {
        if (stack == null || stack.isEmpty()) { DihNotifications.show("Could not read shared item.", ERROR); return; }
        DihItemNbtInspectOverlay ov = DihItemNbtInspectOverlay.getSharedOverlay(font);
        if (ov == null) return;
        ov.open(stack, 60, 60);
    ...
    private void presentOverlay(IDihOverlay ov, boolean background) {
        if (ov == null) return;
    ...
    public int desiredHeight() {
        ...
        if (!dihclient.util.DihDiscordLogin.hasSession()) return 175;
        if (activeTab == Tab.CHAT) {
    ...
    private void renderContextMenu(GuiGraphicsExtractor g, int mx, int my) {
        ...
        if (p.hasLocation) acts.add(new Act("Copy coords", TEXT,
            () -> { copyToClipboard((int) p.x + " " + (int) p.y + " " + (int) p.z); DihNotifications.copied("Coords copied."); }, true));
    ...
    private void chipColored(GuiGraphicsExtractor g, int x, int y, int w, int h, String label, int accentColor,
        ...
        if (clickable || selected) {
            ...
            dihclient.gui.vanillaui.components.Chip.renderDisabled(
                dihclient.gui.vanillaui.UiContexts.overlay(g, font, mx, my),
