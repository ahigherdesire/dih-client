...
public final class Dropdown implements UiComponent {
    ...
    public void render(UiContext context) {
        ...
        for (int rowIndex = 0; rowIndex < visibleRows; rowIndex++) {
            ...
            float hoverT = dihclient.gui.vanillaui.HoverFades.get("dropdown:" + option, hovered);
    ...
    private void prepareInputLayout() {
        if (!open) return;
        int screenWidth = lastScreenWidth > 0 ? lastScreenWidth : Math.max(1, DihUiScale.getVirtualScreenWidth());
        int screenHeight = lastScreenHeight > 0 ? lastScreenHeight : Math.max(1, DihUiScale.getVirtualScreenHeight());
