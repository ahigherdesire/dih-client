...
public final class UiText {
    ...
    public static int width(Font renderer, String value, Identifier fontId, int color) {
        return dihclient.util.DihFonts.ui(renderer).width(value == null ? "" : value);
    }
    ...
    public static String trimToWidth(Font renderer, String value, int maxWidth, Identifier fontId, int color) {
        ...
        TrimKey key = new TrimKey(safeValue, maxWidth, fontId, dihclient.util.DihFonts.enabled());
        ...
        trimmed = dihclient.util.DihFonts.ui(renderer).plainSubstrByWidth(safeValue, Math.max(0, allowedWidth));
    ...
    public static String trimToWidthEllipsis(Font renderer, String value, int maxWidth, Identifier fontId, int color) {
        ...
        String base = dihclient.util.DihFonts.ui(renderer).plainSubstrByWidth(safeValue, Math.max(0, maxWidth - ellipsisWidth));
    ...
    public static void draw(GuiGraphicsExtractor context, Font renderer, String value, Identifier fontId, int color, int x, int y, boolean shadow) {
        context.text(dihclient.util.DihFonts.ui(renderer), value == null ? "" : value, x, y, color, shadow);
    }
    ...
    private record TrimKey(String value, int maxWidth, Identifier fontId, boolean modernFont) {
    }
}
