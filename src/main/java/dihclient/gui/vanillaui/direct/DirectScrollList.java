...
public final class DirectScrollList<T> {
    ...
    private int borderColor = DihColors.secondary();
    private int backgroundColor = DihColors.listBg();
    private int selectedTextColor = DihColors.rowSelectedText();
    private int normalTextColor = 0xFFFFFFFF;
    private int disabledTextColor = DihColors.textDim();
    ...
    private int secondaryTextColor = DihColors.textDim();
    ...
    private void renderContent(GuiGraphicsExtractor ctx, Font textRenderer, double mouseX, double mouseY) {
        viewport.renderSimple(ctx, items.size(), (index, bounds) -> {
            ...
            if (label != null && !label.isEmpty()) {
                ...
                String trimmedLabel = DihText.trimToWidth(textRenderer, label, labelMaxWidth, DihText.Tone.BODY);
                DihText.draw(ctx, textRenderer, trimmedLabel, textColor, textX, textY, false);

                if (badge != null && !badge.isEmpty()) {
                    int labelWidth = DihText.width(textRenderer, trimmedLabel, DihText.Tone.BODY);
                    int badgeX = textX + labelWidth + 4;
                    int badgeColor = badgeColorExtractor != null ? badgeColorExtractor.apply(item) : 0xFFFFAA00;
                    DihText.draw(ctx, textRenderer, badge, badgeColor, badgeX, textY, false);
                }

                if (secondary != null && !secondary.isEmpty()) {
                    int secondaryWidth = textRenderer.width(secondary);
                    int secondaryX = bounds.x + bounds.width - secondaryWidth - 6;
                    DihText.draw(ctx, textRenderer, secondary, secondaryTextColor, secondaryX, textY, false);
                }
            }
        });
    }
