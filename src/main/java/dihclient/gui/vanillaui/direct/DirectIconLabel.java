...
public final class DirectIconLabel extends DirectUiNode {
    ...
    public void render(DirectRenderContext context) {
        ...
        if (icon != null) {
            ...
            context.drawTexturedQuad(dihclient.util.DihThemeTextures.recolored(icon, dihclient.util.DihTheme.Channel.ACCENT), drawX, iconY, drawX + iconSize, iconY + iconSize);
            contentX += iconSize + iconGap;
        }
        UiContexts.textRenderer(context.textRenderer()).draw(context.drawContext(), text, contentX,
            drawY + Math.max(1, (drawH - 9) / 2), context.applyAlpha(dihclient.util.DihTheme.recolor(0xFFF3ECE7, dihclient.util.DihTheme.Channel.TEXT)));
    }
