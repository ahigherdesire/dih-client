...
public class DirectInfoRow extends DirectUiNode {
    ...
    public void render(DirectRenderContext context) {
        ...
        if (hovered) {
            context.drawContext().fill(drawX, drawY, drawX + drawW, drawY + drawH, context.applyAlpha(dihclient.util.DihTheme.recolor(0x14351B1F, dihclient.util.DihTheme.Channel.ACCENT)));
        }
        int labelColor = clickable && hovered
            ? UiSizing.lerpColor(context.theme().color(labelTone), dihclient.util.DihTheme.recolor(0xFFF3ECE7, dihclient.util.DihTheme.Channel.TEXT), 0.35f)
            : context.theme().color(labelTone);
        int valueColor = valueColorOverride != null ? valueColorOverride : context.theme().color(valueTone);
        if (clickable && hovered) {
            valueColor = UiSizing.lerpColor(valueColor, dihclient.util.DihTheme.recolor(0xFFFFF6F6, dihclient.util.DihTheme.Channel.TEXT), 0.28f);
        }
