...
public final class RaceStepSelectorOverlay extends DihOverlayBase {
    ...
    private void renderRow(GuiGraphicsExtractor context, Option option, int x, int y, int width, int index, float mouseX, float mouseY) {
        ...
        int accent = "Condition".equalsIgnoreCase(option.category()) ? DihColors.packetCyan() : DihColors.accent();
        ...
        CompactSurfaces.divider(context, x + 4, y + ROW_HEIGHT - 1, width - 8, DihColors.packetRowDivider());
    }
    ...
    private DihWindowLayout clampToViewport(DihWindowLayout layout) {
        DirectViewport viewport = surface.viewport();
        int margin = 4;
        ...
        return new DihWindowLayout(x, y, width, height, layout.visible, layout.collapsed);
    }
}
