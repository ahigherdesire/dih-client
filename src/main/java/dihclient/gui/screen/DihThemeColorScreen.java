...
public class DihThemeColorScreen extends Screen {
    private static final int FONT_BODY_COLOR = 0xFFF3ECE7;
    private static final int FONT_MUTED = 0xFFB79E9E;

    private final Screen parent;
    private final DihConfig cfg;
    private final DihConfig.ThemeColors pending = new DihConfig.ThemeColors();
    private DihTheme.State pendingState;

    private final Picker picker = new Picker();
    private int selected;

    private DihThemeTextures.Preview logoPreview;
    private DihThemeTextures.Preview panoramaPreview;
    private DihThemeTextures.Preview iconPreview;
    ...
    public DihThemeColorScreen(Screen parent) {
        super(Component.literal("Theme Color"));
        this.parent = parent;
        this.cfg = DihConfig.getGlobal();
    ...
    protected void init() {
        ...
        if (logoPreview == null) {
            logoPreview = DihThemeTextures.preview(
                Identifier.fromNamespaceAndPath("dihclient", "textures/gui/title/dih_client_logo.png"), Channel.ACCENT, 2048);
        }

        if (panoramaPreview == null) {
            panoramaPreview = DihThemeTextures.preview(
                Identifier.fromNamespaceAndPath("dihclient", "textures/gui/title/background/panorama_0.png"), Channel.BACKDROP, 640);
        }

        if (iconPreview == null) {
            iconPreview = DihThemeTextures.preview(UiAssets.ICON_PACKET_CATEGORY, Channel.ACCENT, 128);
        }
    }
    ...
    private void rebuildState() {
        pendingState = DihTheme.State.from(pending);
    }
    ...
    private void renderLeft(UiContext ctx, UiBounds area, int mx, int my) {
        ...
        UiRenderer.frame(g, area, DihColorsBg(), DihThemeBorder());
        int x = area.x() + 12;
        int y = area.y() + 10;
    ...
    private void renderPreview(UiContext ctx, UiBounds area) {
        ...
        draw(g, "DIH Client", UiAssets.FONT_LABEL, p(0xFFFFF4F4, Channel.TEXT), x + 26, y + 8);
    ...
    private int p(int color, Channel ch) {
        return DihTheme.recolor(color, ch, pendingState);
    }
    ...
    private int DihColorsBg() {
        return 0xE60A0A0C;
    }

    private int DihThemeBorder() {
    ...
    public boolean mouseDragged(MouseButtonEvent event, double dx, double dy) {
        picker.mouseDragged(DihUiScale.toVirtualInt(event.x()), DihUiScale.toVirtualInt(event.y()), event.button(), dx, dy);
        return true;
    }

    @Override
    public boolean mouseReleased(MouseButtonEvent event) {
        picker.mouseReleased(DihUiScale.toVirtualInt(event.x()), DihUiScale.toVirtualInt(event.y()), event.button());
        return true;
    }
    ...
    private void setMode(boolean advanced) {
        ...
        if (advanced) {
            int defaultMaster = DihTheme.DEFAULTS[Channel.ACCENT.ordinal()];
            ...
            if (masterChanged) {
                ...
                pending.accent = DihTheme.DEFAULTS[Channel.ACCENT.ordinal()];
                pending.button = DihTheme.DEFAULTS[Channel.BUTTON.ordinal()];
                pending.outline = DihTheme.DEFAULTS[Channel.OUTLINE.ordinal()];
                pending.header = DihTheme.DEFAULTS[Channel.HEADER.ordinal()];
                pending.toggle = DihTheme.DEFAULTS[Channel.TOGGLE.ordinal()];
                pending.hover = DihTheme.DEFAULTS[Channel.HOVER.ordinal()];
                pending.backdrop = DihTheme.DEFAULTS[Channel.BACKDROP.ordinal()];
                pending.danger = DihTheme.DEFAULTS[Channel.DANGER.ordinal()];
            }
        }
    ...
    private void reset() {
        DihConfig.ThemeColors def = new DihConfig.ThemeColors();
        def.advanced = pending.advanced;
        copy(def, pending);
        rebuildState();
        loadSelectedIntoPicker();
        DihNotifications.success("Theme reset to defaults");
    }

    private void apply() {
        copy(pending, cfg.themeColors);
        DihTheme.reload();
        Runnable saved = dihclient.gui.DihThemeApplyOverlay.beginJob("Saving config");
        dihclient.util.DihBackgroundTasks.runTracked("theme-config-save", () -> {
            try {
                cfg.save();
            } finally {
                saved.run();
        ...
        DihNotifications.success("Theme applied");
    }
    ...
    private static void copy(DihConfig.ThemeColors from, DihConfig.ThemeColors to) {
    ...
    private static final class Picker {
        ...
        private DihTheme.State preview;

        private int pp(int color, Channel ch) { return DihTheme.recolor(color, ch, preview); }
        private float hue, sat, bri;
        private int r, g, b, a = 255;
        private UiBounds sv, hueStrip, rSlider, gSlider, bSlider, aSlider, hexField;
        private int drag;
        private DihChatField hexInput;
        ...
        private void ensureHexInput() {
            if (hexInput == null) {
                hexInput = new DihChatField(mc(), mc().font, 0, 0, 20, 16, false);
    ...
    private static final class GradientTex extends AbstractTexture {
        ...
        private GradientTex(Identifier id, String label, int w, int h) {
            ...
            this.texture = RenderSystem.getDevice().createTexture("DIH " + label, 5, GpuFormat.RGBA8_UNORM, w, h, 1, 1);
        ...
        static GradientTex hue() {
            if (hueTex == null) {
                hueTex = new GradientTex(Identifier.fromNamespaceAndPath("dihclient", "dynamic/theme/picker_hue"), "theme picker hue", 14, 96);
        ...
        static GradientTex sv(float hue) {
            ...
            if (svTex == null) svTex = new GradientTex(Identifier.fromNamespaceAndPath("dihclient", "dynamic/theme/picker_sv"), "theme picker sv", 130, 96);
            if (svBucket != bucket) {
