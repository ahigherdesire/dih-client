...
public class DihProxiesScreen extends DihScreen {
    ...
    private DihProxyType type = DihProxyType.Socks5;
    private ProxyFilter filter = ProxyFilter.ALL;
    private DihProxy selectedProxy;
    ...
    private List<DihProxy> cachedFilteredProxies = List.of();
    private long cachedActiveRevision = Long.MIN_VALUE;
    private DihProxy cachedActiveProxy;
    private List<DihProxy> cachedDisplaySource;
    private DihProxy cachedDisplayActive;
    private List<DihProxy> cachedDisplayProxies = List.of();

    public DihProxiesScreen(Screen parent) {
    ...
    protected void init() {
        ...
        DihProxyManager.get().requestGeoLookup(false);
        rebuildButtons();
    }
    ...
    private void rebuildButtons() {
        ...
        DihProxyManager.RefreshStatus refreshStatus = manager.refreshStatus();
        DihProxyManager.ImportStatus importStatus = manager.importStatus();
        ...
        buttons.add(CompactOverlayButton.create(actionX, 96, actionWidths[0], 18, Component.literal("Type: " + type), b -> {
            type = type == DihProxyType.Socks5 ? DihProxyType.Socks4 : DihProxyType.Socks5;
            rebuildButtons();
        ...
        buttons.add(CompactOverlayButton.create(actionX, 96, actionWidths[6], 18, Component.literal("Config"), b -> this.minecraft.gui.setScreen(new DihProxyConfigScreen(this))).setVariant(CompactOverlayButton.Variant.SECONDARY));
        ...
        List<DihProxy> proxies = displayProxies();
    ...
    private ProxyRow buildProxyRow(DihProxy proxy, int rowY, boolean activeSection) {
        ...
        CompactOverlayButton check = CompactOverlayButton.create(checkX, rowY + 7, ROW_CHECK_W, 16, Component.literal(proxy.status == DihProxy.Status.CHECKING ? "..." : "Check"), b -> checkProxy(proxy));
        check.setVariant(CompactOverlayButton.Variant.PRIMARY);
        check.active = proxy.status != DihProxy.Status.CHECKING;
    ...
    public void tick() {
        ...
        proxyListScrollOffset = clampScrollOffset(proxyListScrollOffset, maxScroll);
        proxyListScroll.jumpTo(proxyListScrollOffset, maxScroll);
        DihProxyManager.RefreshStatus status = DihProxyManager.get().refreshStatus();
        if (status.revision() != lastRefreshRevision) {
            lastRefreshRevision = status.revision();
            rebuildButtons();
        }
        DihProxyManager.ImportStatus importStatus = DihProxyManager.get().importStatus();
    ...
    private void addProxy() {
        DihProxy proxy = proxyFromFields();
        if (proxy == null) return;
        if (DihProxyManager.get().add(proxy)) {
            selectedProxy = null;
            clearProxyFields();
    ...
    private void updateProxy() {
        ...
        DihProxy updated = proxyFromFields();
        if (updated == null) return;
        if (DihProxyManager.get().update(selectedProxy, updated)) {
    ...
    private void toggleProxy(DihProxy proxy) {
        if (proxy == null) return;
        int sectionsBefore = displayProxies().size() - filteredProxies().size();
        DihProxyManager.get().setEnabled(proxy, !proxy.enabled);
    ...
    private void deleteProxy(DihProxy proxy) {
        if (proxy == null) return;
        DihProxyManager.get().remove(proxy);
        if (proxy.equals(selectedProxy)) selectedProxy = null;
        toast("Deleted " + proxy.displayName() + ".", MUTED);
        rebuildButtons();
    }

    private void checkProxy(DihProxy proxy) {
        if (proxy == null || proxy.status == DihProxy.Status.CHECKING) return;
        proxy.status = DihProxy.Status.CHECKING;
        proxy.latency = 0L;
        rebuildButtons();
        DihBackgroundTasks.runTracked("Dih-Proxy-Check", () -> {
            proxy.checkStatus(DihProxyManager.get().getTimeoutMs());
    ...
    private void refreshProxies() {
        ...
        DihProxyManager.RefreshStatus currentStatus = manager.refreshStatus();
        ...
        rebuildButtons();
        DihBackgroundTasks.watchUntil("Dih-Proxy-Refresh-Watch", () -> !manager.refreshStatus().running(), () -> {
            if (this.minecraft != null) {
                this.minecraft.execute(() -> {
                    DihProxyManager.RefreshStatus finished = manager.refreshStatus();
    ...
    private void importProxies() {
        DihProxyManager manager = DihProxyManager.get();
        DihProxyManager.ImportStatus status = manager.importStatus();
        if (status.running()) {
    ...
    private void watchImport(DihProxyManager manager, long generation) {
        toast("Importing proxies...", DEFAULT_COLOR);
        rebuildButtons();
        DihBackgroundTasks.watchUntil("Dih-Proxy-Import-Watch", () -> !manager.importStatus().running(), () -> {
            if (this.minecraft != null) {
                this.minecraft.execute(() -> {
                    DihProxyManager.ImportStatus finished = manager.importStatus();
    ...
    private DihProxy proxyFromFields() {
    ...
    public void extractRenderState(GuiGraphicsExtractor graphics, int mouseX, int mouseY, float delta) {
        ...
        try {
            ...
            List<DihProxy> proxies = filteredProxies();
            DihProxyManager manager = DihProxyManager.get();
    ...
    private void selectProxy(DihProxy proxy) {
        selectedProxy = proxy;
        if (proxy != null) {
            ...
            type = proxy.type == null ? DihProxyType.Socks5 : proxy.type;
        }
        rebuildButtons();
    }

    private List<DihProxy> filteredProxies() {
        DihProxyManager manager = DihProxyManager.get();
        ...
        List<DihProxy> proxies = manager.all();
        List<DihProxy> filtered;
    ...
    private List<DihProxy> displayProxies() {
        List<DihProxy> filtered = filteredProxies();
        DihProxy active = activeProxy();
        if (filtered == cachedDisplaySource && active == cachedDisplayActive) return cachedDisplayProxies;
        List<DihProxy> display = filtered;
    ...
    private boolean matchesFilter(DihProxy proxy) {
        return switch (filter) {
            case ALL -> true;
            case ALIVE -> proxy.status == DihProxy.Status.ALIVE;
            case DEAD -> proxy.status == DihProxy.Status.DEAD;
            case UNCHECKED -> proxy.status == DihProxy.Status.UNCHECKED || proxy.status == DihProxy.Status.CHECKING;
            case ENABLED -> proxy.enabled;
        };
    }

    private boolean matchesSearch(DihProxy proxy, String query) {
    ...
    private String statusText(DihProxy proxy) {
        if (proxy == null) return "";
    ...
    private String refreshStatusText(DihProxyManager.RefreshStatus status) {
    ...
    private String importStatusText(DihProxyManager.ImportStatus status) {
        if (status == null) return "";
    ...
    private void renderWorkProgress(GuiGraphicsExtractor graphics, DihProxyManager.RefreshStatus refreshStatus, DihProxyManager.ImportStatus importStatus) {
    ...
    private record ProxyRow(DihProxy proxy, int y, boolean activeSection, CompactOverlayButton toggleButton, CompactOverlayButton checkButton, CompactOverlayButton deleteButton) {
    }
}
