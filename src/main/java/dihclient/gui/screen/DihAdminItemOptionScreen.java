...
public final class DihAdminItemOptionScreen extends DihScreen {
    ...
    public DihAdminItemOptionScreen(Screen parent, Module module, Setting<?, ?> option, Mode mode) {
    ...
    public boolean mouseReleased(MouseButtonEvent event) {
        return lastDirectContext == null || editor.mouseReleased(lastDirectContext,
            DihUiScale.toVirtualInt(event.x()), DihUiScale.toVirtualInt(event.y()), event.button());
    }

    @Override
    public boolean mouseDragged(MouseButtonEvent event, double dx, double dy) {
        return lastDirectContext == null || editor.mouseDragged(lastDirectContext,
            DihUiScale.toVirtualInt(event.x()), DihUiScale.toVirtualInt(event.y()), event.button(), (float) dx, (float) dy);
    }

    @Override
    public boolean mouseScrolled(double mouseX, double mouseY, double scrollX, double scrollY) {
        return lastDirectContext == null || editor.mouseScrolled(lastDirectContext,
            DihUiScale.toVirtualInt(mouseX), DihUiScale.toVirtualInt(mouseY), (float) scrollY);
    }
    ...
    private void save() {
        if (module != null && option != null) {
            ...
            if ("nbt-item-components".equals(option.id()) && module instanceof BuiltinModules.AdminToolsModule adminTools) {
                ...
                if (!adminTools.setRawItemStackSnbt(value)) {
                    ...
                    DihNotifications.error("Invalid full ItemStack SNBT.");
                    return;
                }
            } else {
        ...
        DihNotifications.show(mode.title + " updated.", 0xFF35D873);
        onClose();
    }

    private boolean validateFullNbt() {
        if (mode != Mode.FULL_NBT) return true;
        var stack = DihItemCommandSerializer.itemStackFromSnbt(editor.text());
        if (stack.isEmpty()) {
            validationOk = false;
            validationStatus = "Invalid ItemStack SNBT";
            DihNotifications.error(validationStatus);
            return false;
        }
        String error = DihItemCommandSerializer.validationError(stack);
        validationOk = error.isBlank();
        validationStatus = validationOk ? "Valid complete ItemStack" : error;
        if (validationOk) DihNotifications.show("Full ItemStack NBT is valid.", 0xFF35D873);
        else DihNotifications.error(error);
        return validationOk;
    }

    private void formatFullNbt() {
        if (mode != Mode.FULL_NBT) return;
        String repaired = dihclient.util.SnbtRepair.repair(editor.text());
        var stack = DihItemCommandSerializer.itemStackFromSnbt(repaired);
        if (stack.isEmpty()) {
            editor.setText(DihItemNbtInspector.prettySnbt(repaired));
            validationOk = false;
            validationStatus = "Formatted (structure repaired — still not a valid ItemStack; check ids/values)";
            return;
        }
        String error = DihItemCommandSerializer.validationError(stack);
        editor.setText(DihItemNbtInspector.prettySnbt(DihItemCommandSerializer.itemStackSnbt(stack)));
        validationOk = error.isBlank();
    ...
    private void copyRaw() {
        ...
        DihNotifications.copied(selected.isEmpty() ? "Copied full ItemStack NBT." : "Copied selection.");
    }
    ...
    private String decode(String raw) {
        ...
        return switch (mode) {
            ...
            case FULL_NBT -> DihItemNbtInspector.prettySnbt(value);
            default -> value;
        };
    }
