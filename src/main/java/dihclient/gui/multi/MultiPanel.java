...
public final class MultiPanel {
    public interface Host {
        ...
        void editMacro(DihMacro macro, Consumer<DihMacro> onSaved);
        void editFormValues(MultiProfile profile, Set<String> selectedAccounts, Consumer<MultiProfile> onSaved);
        void editAutoAccept(dihclient.util.multi.MultiAutoAccept config,
    ...
    private final EnumSet<DihAccountType> accountFilters = EnumSet.allOf(DihAccountType.class);
    ...
    private List<DihMacro> cachedMacros = List.of();
    private int cachedAccountSourceSize = -1;
    private long cachedAccountSourceRevision = Long.MIN_VALUE;
    private List<DihAccount> cachedAccountSource = List.of();
    ...
    public void render(GuiGraphicsExtractor g, int x, int y, int w, int h, int mx, int my, float partial) {
        ...
        if (macroRowTooltip != null && dihclient.util.DihConfig.getGlobal().multiShowTooltips) {
    ...
    private void renderAccounts(GuiGraphicsExtractor g, int mx, int my) {
        ...
        y += 20;
        DihAccountType[] types = {DihAccountType.Cracked, DihAccountType.Session,
            DihAccountType.Microsoft, DihAccountType.TheAltening, DihAccountType.Generated};
    ...
    private void renderMacros(GuiGraphicsExtractor g, int mx, int my) {
        ...
        y += 20;
        List<DihMacro> macros = filteredMacros();
        ...
        button(g, dx + third + 3, cy, third, 17, "Edit", border(), text(), mx, my,
            selectedMacroName.isBlank() ? null : () -> editMacro(DihMacroManager.get().get(selectedMacroName)));
    ...
    private void runMacros() {
        ...
        if (m.hasAssignedMacroOnInteractiveScope(selectedSessionIds)) {
            ...
            DihMacro macro = DihMacroManager.get().get(selectedMacroName);
    ...
    private String assignAccountName(String id) {
        ...
        DihAccount account = DihAccountManager.get().findById(id);
    ...
    private void editMacro(DihMacro macro) {
    ...
    private void confirmMacroDelete() {
        DihMacro macro = DihMacroManager.get().get(pendingMacroDelete);
        String name = pendingMacroDelete;
        pendingMacroDelete = "";
        if (macro == null) return;
        DihMacroManager.get().delete(macro);
    ...
    public boolean mouseClicked(int mx, int my, int button) {
        ...
        if (button == 0) {
            for (int i = hotspots.size() - 1; i >= 0; i--) {
                ...
                try { hit.action().run(); }
                catch (Throwable error) { DihNotifications.show("Multi action failed", danger()); }
                return true;
            }
    ...
    private void button(GuiGraphicsExtractor g, int x, int y, int w, int h, String label, int outline, int color,
        ...
        dihclient.gui.vanillaui.components.Button.Tone tone = outline == success() ? dihclient.gui.vanillaui.components.Button.Tone.SUCCESS
            : outline == danger() ? dihclient.gui.vanillaui.components.Button.Tone.DANGER
    ...
    private void scrollbar(GuiGraphicsExtractor g, Viewport viewport, int offset, int maxOffset, int mx, int my,
        ...
        dihclient.gui.vanillaui.components.Scrollbar.render(
            dihclient.gui.vanillaui.UiContexts.overlay(g, font, mx, my),
            new dihclient.gui.vanillaui.components.Scrollbar.Metrics(
    ...
    private void slider(GuiGraphicsExtractor g, int x, int y, int w, int h, String label,
        ...
        dihclient.gui.vanillaui.components.Slider.render(
            dihclient.gui.vanillaui.UiContexts.overlay(g, font, mx, my),
    ...
    private List<AccountChoice> filteredAccounts(MultiProfile profile) {
        ...
        int filterMask = 0;
        for (DihAccountType filter : accountFilters) filterMask |= 1 << filter.ordinal();

        long accountRevision = DihAccountManager.get().changeRevision();
        if (cachedAccountSourceSize < 0 || accountRevision != cachedAccountSourceRevision) {
            DihAccountManager accountManager = DihAccountManager.get();
        ...
        String defaultName = DihAccountSessionSwitcher.getOriginalUser().getName();
        AccountChoice current = new AccountChoice(MultiProfile.DEFAULT_ACCOUNT_ID, defaultName + " (Default)",
            DihAccountSessionSwitcher.getOriginalUser().getAccessToken().isBlank() ? DihAccountType.Cracked : DihAccountType.Session,
        ...
        for (DihAccount account : cachedAccountSource) {
        ...
        for (MultiProfile.SessionSpec spec : shownSessions) {
            ...
            if (!known && (query.isBlank() || spec.accountId().toLowerCase(Locale.ROOT).contains(query))) {
                out.add(new AccountChoice(spec.accountId(), "Missing: " + spec.accountId(), DihAccountType.Cracked, false));
            }
        }
    ...
    private List<DihMacro> filteredMacros() {
    ...
    private String proxyLabel(MultiProfile profile, String id) {
        ...
        DihProxy proxy = DihProxyManager.get().findById(id);
    ...
    private int border() { return DihTheme.recolor(BORDER_STOCK, DihTheme.Channel.OUTLINE); }
    private int accent() { return DihTheme.recolor(ACCENT_STOCK, DihTheme.Channel.ACCENT); }
    private int success() { return DihTheme.recolor(SUCCESS_STOCK, DihTheme.Channel.SUCCESS); }
    private int danger() { return DihTheme.recolor(DANGER_STOCK, DihTheme.Channel.DANGER); }
    private int text() { return DihTheme.recolor(TEXT_STOCK, DihTheme.Channel.TEXT); }
    private int muted() { return DihTheme.recolor(MUTED_STOCK, DihTheme.Channel.TEXT); }
    private int warn() { return DihTheme.recolor(WARN_STOCK, DihTheme.Channel.ACCENT); }
    ...
    private record AccountChoice(String id, String label, DihAccountType type, boolean current) { }
}
