...
public class DihMacro {
    ...
    public DihMacro(String name) {
        this.name = name;
    }

    public DihMacro deepCopy() {
        return new DihMacro().fromTag(this.toTag());
    }

    public DihMacro deepCopy(String newName) {
        DihMacro copy = deepCopy();
    ...
    public DihMacro sanitizeForSharing() {
        if (actions != null) {
    ...
    public DihMacro fromTag(CompoundTag tag) {
    ...
    public static MacroAction createActionFromTag(CompoundTag actionTag) {
        ...
        try {
            ...
            String typeName = actionTag.getStringOr("type", "");
            dihclient.DihClientAddon.LOG.warn("[Macros] Failed to deserialize action '{}'; preserving its data", typeName, t);
    ...
    private static MacroAction recoverUnknownAction(CompoundTag actionTag) {
        ...
        java.util.function.Supplier<MacroAction> factory = dihclient.api.macro.MacroActionRegistry.factory(typeName);
        if (factory != null) {
            ...
            try {
                ...
                dihclient.DihClientAddon.LOG.warn("[MacroActions] Addon action '{}' failed to deserialize; keeping as placeholder", typeName, t);
            }
        }
