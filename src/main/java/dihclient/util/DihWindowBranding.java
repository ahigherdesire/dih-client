...
public final class DihWindowBranding {
    public static final String WINDOW_TITLE = "DIH Client";
    private static final String ICON_ROOT = "assets/dihclient/icons/window/";
    ...
    private static void applyPending(Minecraft client) {
        ...
        boolean hidden = dihclient.modules.PackHideState.isActive() || DihLiteVariant.enabled();
        boolean ok;
        if (hidden) {
    ...
    private static InputStream openIconStream(String fileName) throws IOException {
        ...
        if (stream == null) {
            throw new IOException("Missing Dih window icon resource: " + fileName);
        }
        return stream;
    }
}
