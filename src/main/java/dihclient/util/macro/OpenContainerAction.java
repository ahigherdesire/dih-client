...
public class OpenContainerAction implements MacroAction, WaitsForGui, RaycastAim {
    ...
    public RaycastAim.Target raycastTarget(Minecraft mc) {
        ...
        DihContainerTarget target = targetMode == TargetMode.LAST_TARGET
            ? DihSharedState.get().getLastContainerTarget()
            : firstEntityTarget();
    ...
    private DihContainerTarget firstEntityTarget() {
    ...
    public boolean tryExecute(Minecraft mc) {
        ...
        DihContainerTarget target = targetMode == TargetMode.BLOCK
            ? DihContainerTarget.forBlock(blockPos)
            : DihSharedState.get().getLastContainerTarget();
    ...
    public void captureCurrentLookTarget(Minecraft mc) {
        ...
        if (mc.hitResult instanceof EntityHitResult entityHit && entityHit.getEntity() != null) {
            ...
            entityTarget = DihContainerTarget.toSpecificEntityRef(entityHit.getEntity());
    ...
    private static String entityTargetLabel(String targetRef) {
        ...
        return DihRegistryLabels.entity(targetRef);
    }
