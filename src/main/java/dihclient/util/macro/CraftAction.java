...
public class CraftAction implements MacroAction {
    public static final class CraftEntry {
        ...
        public static CraftEntry fromOption(DihCraftingHelper.CraftableRecipeOption option, int amount, boolean useMaxAmount) {
            CraftEntry entry = new CraftEntry();
    ...
    public void execute(Minecraft mc) {
        if (mc.player == null || mc.getConnection() == null) {
            DihClientMessaging.sendPrefixed("No network connection!");
            return;
        }
        if (!hasEntries()) {
            DihClientMessaging.sendPrefixed("No craft recipe selected!");
            return;
        }
        ...
        for (CraftEntry entry : entries) {
            ...
            if (option == null) {
                DihClientMessaging.sendPrefixed("§cRecipe not found: " + entry.resultName);
                return;
            }
