...
public class HClipAction implements MacroAction {
    ...
    private static final boolean DEBUG_ROUTES = Boolean.getBoolean("dih.hclip.debug");
    ...
    public static Result perform(Minecraft mc, Options options) {
        ...
        if (mc == null || mc.player == null || mc.getConnection() == null) {
            if (options.showMessage) DihClientMessaging.sendPrefixed("§cHClip: no world / connection.");
        ...
        if (options.mode != Mode.MANUAL) {
            ...
            if (!target.success()) {
                if (options.showMessage) DihClientMessaging.sendPrefixed("§cHClip: " + target.message());
        ...
        if (!plan.success()) {
            if (options.showMessage) DihClientMessaging.sendPrefixed("§cHClip: " + plan.message());
        ...
        if (vehicle != null) {
            if (plan.requiresVertical(player.getY())) {
                ...
                if (options.showMessage) DihClientMessaging.sendPrefixed("§cHClip: " + message);
