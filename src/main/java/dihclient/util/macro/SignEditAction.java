...
public class SignEditAction implements MacroAction, WaitsForGui {
    ...
    private BlockPos resolvePos(Minecraft mc) {
        ...
        if (targetMode == TargetMode.LAST_INTERACTED_BLOCK) return DihSharedState.get().getLastInteractedBlockPos();
        if (mc.gui.screen() instanceof DihSignEditAccess access) return access.dih$getSignPos();
        return null;
    }

    private boolean resolveFront(Minecraft mc) {
        if (targetMode == TargetMode.CURRENT_SIGN_GUI && mc.gui.screen() instanceof DihSignEditAccess access) return access.dih$isFrontText();
        return frontText;
    }
