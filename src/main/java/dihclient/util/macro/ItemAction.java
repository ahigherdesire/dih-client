...
public class ItemAction implements MacroAction, WaitsForGui {
    ...
    private static final DihDropAction[] ACTIONS = DihDropAction.values();
    private static final int[] ITEM_CLICK_ACTIONS = {
        DihDropAction.PICKUP.ordinal(),
        DihDropAction.QUICK_MOVE.ordinal(),
        DihDropAction.SWAP.ordinal(),
        DihDropAction.CLONE.ordinal(),
        DihDropAction.DROP_ITEM.ordinal(),
        DihDropAction.DROP_STACK.ordinal()
    };
    private static final int[] LEGACY_ITEM_CLICK_ACTIONS = {
        DihDropAction.PICKUP.ordinal(),
        DihDropAction.PICKUP_ALL.ordinal(),
        DihDropAction.QUICK_MOVE.ordinal(),
        DihDropAction.SWAP.ordinal(),
        DihDropAction.CLONE.ordinal(),
        DihDropAction.DROP_ITEM.ordinal(),
    ...
    public DihDropAction getItemAction(int index) {
        return ACTIONS[getItemActionIdx(index)];
    }

    public String getItemButtonName(int index) {
        if (getItemAction(index) == DihDropAction.PICKUP_ALL) return "All";
        if (getItemAction(index) == DihDropAction.SWAP) {
    ...
    public void cycleItemButton(int index) {
        ...
        if (getItemAction(index) == DihDropAction.PICKUP_ALL) return;
    ...
    public static java.util.List<DihDropAction> itemClickActions() {
        java.util.List<DihDropAction> list = new java.util.ArrayList<>();
    ...
    public int getItemClickActionOptionIndex(int index) {
        ...
        java.util.List<DihDropAction> opts = itemClickActions();
    ...
    public java.util.List<String> itemButtonOptions(int index) {
        DihDropAction action = getItemAction(index);
    ...
    private static String itemButtonLabelFor(DihDropAction action, int b) {
        if (action == DihDropAction.PICKUP_ALL) return "All";
        if (action == DihDropAction.SWAP) return "H" + (Math.max(0, Math.min(8, b)) + 1);
    ...
    private record ResolvedClickEntry(DihDropAction action, int button, int slot, ItemTarget cursorTarget, int count) {}
    ...
    private List<ResolvedClickEntry> resolveClickEntries(Minecraft mc, AbstractContainerMenu handler) {
        ...
        if (!itemNames.isEmpty()) {
            for (int ei = 0; ei < itemNames.size(); ei++) {
                ...
                DihDropAction eiAction = getItemAction(ei);
                ...
                if (eiAction == DihDropAction.PICKUP_ALL && useCursorItemForPickupAll) {
                    out.add(new ResolvedClickEntry(eiAction, eiButton, -1, entryTarget, clickCount));
                    continue;
                }
                int slotToUse;
                if (configuredEntrySlot >= 0 && !entryName.isEmpty()) {
                    int resolvedSlot = DihInventoryHelper.resolveConfiguredHandlerSlot(mc, configuredEntrySlot);
                    if (resolvedSlot < 0) continue;
                    slotToUse = findExactMatchingSlot(handler, entryTarget, resolvedSlot);
                } else if (configuredEntrySlot >= 0) {
                    slotToUse = DihInventoryHelper.resolveConfiguredHandlerSlot(mc, configuredEntrySlot);
                } else if (useSlot && targetSlot >= 0) {
                    int configuredSlot = DihInventoryHelper.resolveConfiguredHandlerSlot(mc, targetSlot);
    ...
    private static void performEntry(Minecraft mc, ResolvedClickEntry e) {
        boolean cursorPickupAll = e.cursorTarget() != null
                || (e.action() == DihDropAction.PICKUP_ALL && e.slot() < 0);
        if (cursorPickupAll) {
            DihCursorClickHelper.click(mc, e.cursorTarget(), e.count());
            return;
        }
        if (e.action() == DihDropAction.DROP_ITEM || e.action() == DihDropAction.DROP_STACK) {
            DihDropHelper.dropFromHandlerSlot(mc, e.slot(), e.action() == DihDropAction.DROP_STACK ? 0 : e.count());
            return;
        }
        for (int j = 0; j < e.count(); j++) {
            DihInventoryClickHelper.click(mc, e.slot(), e.button(), e.action().toContainerInput());
        }
    }
    ...
    public List<Runnable> buildClickPlan(Minecraft mc) {
        ...
        for (ResolvedClickEntry e : resolveClickEntries(mc, handler)) {
            ...
            if (cursorPickupAll) {
                ...
                for (int j = 0; j < e.count(); j++) plan.add(() -> DihCursorClickHelper.click(mc, t, 1));
            } else if (e.action() == DihDropAction.DROP_STACK) {
                final int fs = e.slot();
                plan.add(() -> DihDropHelper.dropFromHandlerSlot(mc, fs, 0));
            } else if (e.action() == DihDropAction.DROP_ITEM) {
                final int fs = e.slot();
                for (int j = 0; j < e.count(); j++) plan.add(() -> DihDropHelper.dropFromHandlerSlot(mc, fs, 1));
            } else {
                final int fs = e.slot();
                final int fb = e.button();
                final net.minecraft.world.inventory.ContainerInput input = e.action().toContainerInput();
                for (int j = 0; j < e.count(); j++) plan.add(() -> DihInventoryClickHelper.click(mc, fs, fb, input));
            }
        }
        return plan;
    }
    ...
    private int findMatchingSlotInPartition(AbstractContainerMenu handler, ItemTarget target, StackAmountMode mode, Boolean playerInventoryOnly) {
        ...
        for (Slot slot : handler.slots) {
            ...
            if (playerInventoryOnly != null) {
                boolean isPlayerInventory = DihInventoryHelper.isInventorySlot(MC, slot);
    ...
    private int findExactMatchingSlot(AbstractContainerMenu handler, ItemTarget target, int requiredSlot) {
        ...
        int visibleSlot = DihInventoryHelper.toUserVisibleSlot(MC, requiredSlot);
    ...
    public DihDropAction getAction() {
    ...
    public String getButtonName() {
        if (getAction() == DihDropAction.PICKUP_ALL) return "All";
        if (getAction() == DihDropAction.SWAP) {
    ...
    public void cycleButton() {
        if (getAction() == DihDropAction.PICKUP_ALL) return;
    ...
    private static boolean usesFixedButtonForItemClick(DihDropAction action) {
        return action != DihDropAction.QUICK_MOVE && action.usesFixedButton();
    }

    private static int buttonLimitForItemClick(DihDropAction action) {
    ...
    private static int normalizeButtonForItemClick(DihDropAction action, int rawButton) {
        if (action == null) action = DihDropAction.PICKUP;
    ...
    private static int normalizeItemClickActionIndex(int rawIndex) {
        ...
        return DihDropAction.PICKUP.ordinal();
    }
