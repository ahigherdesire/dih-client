...
public class SwapSlotsAction implements MacroAction {
    ...
    public void execute(Minecraft mc) {
        ...
        if (fromUseItemName) {
            ...
            resolvedFrom = DihInventoryHelper.findUserVisibleSlot(mc, target);
        ...
        if (toUseItemName) {
            ...
            if (target == null) return;
            if (!target.hasSlot() && !target.hasIdentity()) return;
            resolvedTo = DihInventoryHelper.findUserVisibleSlot(mc, target);
            if (resolvedTo == -1) return;
        }

        int fromHandlerSlot = DihInventoryHelper.resolveConfiguredHandlerSlot(mc, resolvedFrom);
        int toHandlerSlot = DihInventoryHelper.resolveConfiguredHandlerSlot(mc, resolvedTo);
        if (fromHandlerSlot < 0 || toHandlerSlot < 0) return;

        DihInventoryHelper.swapHandlerSlots(mc, fromHandlerSlot, toHandlerSlot);
    }
