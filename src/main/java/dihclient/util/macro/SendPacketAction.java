...
import dihclient.util.DihPacketSender;
...
public class SendPacketAction implements MacroAction, WaitsForGui {
    public List<DihSharedState.QueuedPacket> packets = new ArrayList<>();
    ...
    public List<DihSharedState.QueuedPacket> getPackets() {
        return packets;
    }

    public void regeneratePackets() {
        List<DihSharedState.QueuedPacket> newPackets = new ArrayList<>(packets.size());
        for (DihSharedState.QueuedPacket qp : packets) {
            if (qp.packet != null) {
                if (qp.isExactReplay()) {
                    newPackets.add(new DihSharedState.QueuedPacket(qp.packet, qp.getDelay(), qp.getId(), qp.getReplayMode()));
                    continue;
                }
                Packet<?> regenerated = PacketRegenerator.regenerate(qp.packet);
                if (regenerated != null) {

                    newPackets.add(new DihSharedState.QueuedPacket(regenerated, qp.getDelay(), qp.getId(), qp.getReplayMode()));
                }
    ...
    public void execute(Minecraft mc) {
        ...
        for (DihSharedState.QueuedPacket qp : packets) {
            ...
            try {
                DihPacketSender.send(qp.packet);
                successCount++;
            } catch (Exception e) {
        ...
        if (failCount > 0) {
            DihClientMessaging.sendPrefixed(String.format("§eSent %d/%d packets (%d failed)",
            ...
            if (packetList != null) {
                for (int i = 0; i < packetList.size(); i++) {
                    ...
                    if (element instanceof CompoundTag) {
                        DihSharedState.QueuedPacket qp = DihClipboardHelper.deserializeQueuedPacket((CompoundTag) element);
                        if (qp != null) {
                            packets.add(qp);
                        }
                    }
    ...
    public String getDisplayName() {
        ...
        for (DihSharedState.QueuedPacket qp : packets) {
            if (qp.packet != null) {
                String name = DihPacketNamer.getFriendlyName(qp.packet);
