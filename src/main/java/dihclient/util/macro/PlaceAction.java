...
import dihclient.util.DihMacroSneak;
...
public class PlaceAction implements MacroAction, WaitsForGui, PacketOrdered, RaycastAim {
    ...
    public void execute(Minecraft mc) {
        ...
        if (!DihContainerTarget.isWithinBlockReach(mc, blockPos)) return;
        ...
        if (sneaking) DihMacroSneak.hold(mc, sneakMode);
        try {
            mc.gameMode.useItemOn(mc.player, InteractionHand.MAIN_HAND, hit);
        } finally {
            if (sneaking) DihMacroSneak.release(mc, sneakMode);
        }
    ...
    static BlockHitResult resolvePlaceHit(Minecraft mc, BlockPos targetPos, boolean manualDirection, Direction manualFace) {
        ...
        for (Direction face : order) {
            ...
            if (!DihContainerTarget.isWithinBlockReach(mc, supportPos)) continue;
    ...
    private static void selectItemForPlace(Minecraft mc, ItemTarget target) {
        ...
        if (target.hasIdentity()) {
            DihInventoryHelper.selectHotbarItem(mc, target, mc.player.getInventory().getSelectedSlot());
            return;
        }

        int visibleSlot = target.slot;
        if (visibleSlot >= 0 && visibleSlot <= 8) {
            DihInventoryHelper.selectHotbarSlot(mc, visibleSlot);
            return;
        }

        int handlerSlot = DihInventoryHelper.resolveConfiguredHandlerSlot(mc, visibleSlot);
        if (handlerSlot < 0) return;

        int hotbarSlot = mc.player.getInventory().getSelectedSlot();
        int hotbarHandlerSlot = DihInventoryHelper.resolveConfiguredHandlerSlot(mc, hotbarSlot);
        if (hotbarHandlerSlot >= 0) {
            DihInventoryHelper.swapHandlerSlots(mc, handlerSlot, hotbarHandlerSlot);
        }
    }
