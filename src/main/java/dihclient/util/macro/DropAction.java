...
public class DropAction implements MacroAction, WaitsForGui, PacketOrdered {
            ...
            if (mode == DropMode.ALL) {
                ...
                for (int i = 0; i < 36; i++) {
                    if (!mc.player.getInventory().getItem(i).isEmpty()) {
                        if (useHandlerSlots) {
                            int resolved = DihInventoryHelper.toHandlerSlot(mc, i);
                ...
                if (useHandlerSlots) {
                    int resolved = DihInventoryHelper.toHandlerSlot(mc, selected);
        ...
        if (!targets.isEmpty()) {
            for (int ei = 0; ei < targets.size(); ei++) {
                ...
                if (configuredSlot >= 0 && !entryName.isEmpty()) {
                    ...
                    if (useHandlerSlots) {
                        int resolvedSlot = DihInventoryHelper.resolveConfiguredHandlerSlot(mc, configuredSlot);
    ...
    private void dropMatchingItemInSpecificSlot(Minecraft mc, int configuredSlot, ItemTarget target, int count) {
        ...
        if (useHandlerSlots) {
            ...
            int visibleSlot = DihInventoryHelper.toUserVisibleSlot(mc, resolvedSlot);
    ...
    private void dropFromInvSlot(Minecraft mc, int invSlot, int count) {
        DihDropHelper.dropFromInventorySlot(mc, invSlot, count);
    }

    private void dropFromHandlerSlot(Minecraft mc, int handlerSlotId, int count) {
        DihDropHelper.dropFromHandlerSlot(mc, handlerSlotId, count);
    }
