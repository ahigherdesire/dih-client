...
public class RaceAction implements MacroAction {
    ...
    private transient DihPacketClick.Target legacyTarget = null;
    ...
    public void fromTag(CompoundTag tag) {
        ...
        MacroAction nested = tag.getCompound("triggerAction")
            .map(dihclient.util.DihMacro::createActionFromTag)
            .orElse(null);
        ...
        if (tag.contains("triggerType")) {
            ...
            legacyTarget = tag.getCompound("target").map(DihPacketClick.Target::fromTag).orElse(null);
        }
    ...
    private static MacroAction triggerFromFlatFields(CompoundTag tag) {
        ...
        CompoundTag actionTag = DihMacro.serializeAction(action);
        switch (type) {
            case DELAY -> {
    ...
    private static void writeFlatTriggerFields(CompoundTag tag, MacroAction action) {
        ...
        CompoundTag src = DihMacro.serializeAction(action);
