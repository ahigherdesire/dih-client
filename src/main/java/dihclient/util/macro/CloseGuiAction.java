    ...
    public void execute(Minecraft mc) {
        ...
        if (useItemFilter && (target.hasSlot() || target.hasIdentity())) {
            ...
            if (mc.player.containerMenu != null) {
                ...
                int resolvedTargetSlot = DihInventoryHelper.resolveConfiguredHandlerSlot(mc, effectiveSlot);
                if (resolvedTargetSlot >= 0 && resolvedTargetSlot < mc.player.containerMenu.slots.size()) {
                    ...
                    int visibleSlot = DihInventoryHelper.toUserVisibleSlot(mc, resolvedTargetSlot);
        ...
        if (matchGui && matchItem) {
            DihGuiActions.closeCurrentScreen(mc, sendPacket, false);
        }
    }
