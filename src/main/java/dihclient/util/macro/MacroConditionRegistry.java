...
public class MacroConditionRegistry {
    ...
    public static CompletableFuture<Void> waitForGamemodeChange(WaitGamemodeChangeAction action) {
        CompletableFuture<Void> future = new CompletableFuture<>();
        GamemodeChangeCondition condition = new GamemodeChangeCondition(action, DihFakeGamemode.snapshot(), future);
        pendingConditions.add(condition);
        Minecraft mc = Minecraft.getInstance();
        if (checkOnClientThread(mc, condition)) {
            condition.complete();
    ...
    public static CompletableFuture<Void> await(dihclient.api.macro.AddonCondition condition) {
        CompletableFuture<Void> future = new CompletableFuture<>();
        if (condition == null) {
    ...
    static final class AddonConditionAdapter implements PendingCondition {
        private final dihclient.api.macro.AddonCondition condition;
        private final CompletableFuture<Void> future;

        AddonConditionAdapter(dihclient.api.macro.AddonCondition condition, CompletableFuture<Void> future) {
    ...
    static class GamemodeChangeCondition implements PendingCondition {
        ...
        final DihFakeGamemode.Snapshot baseline;
        final CompletableFuture<Void> future;
        private volatile boolean cancelled = false;

        GamemodeChangeCondition(WaitGamemodeChangeAction action, DihFakeGamemode.Snapshot baseline, CompletableFuture<Void> future) {
            this.action = action == null ? new WaitGamemodeChangeAction() : action;
            this.baseline = baseline == null ? DihFakeGamemode.snapshot() : baseline;
            this.future = future;
        }

        @Override
        public boolean check(Minecraft mc) {
            DihFakeGamemode.Snapshot now = DihFakeGamemode.snapshot();
    ...
    public static CompletableFuture<Void> waitForSlotChange(WaitForSlotChangeAction action) {
        ...
        if (mc.player != null) {
            ...
            for (int i = 0; i < action.entries.size(); i++) {
                ...
                int handlerSlot = DihInventoryHelper.resolveConfiguredHandlerSlot(mc, slotNum);
                ...
                if (e.waitMode == WaitForSlotChangeAction.WaitMode.IS_EMPTY) {
                    ...
                    if (handler != null) {
                        for (net.minecraft.world.inventory.Slot s : handler.slots) {
                            ...
                            int visibleSlot = DihInventoryHelper.toUserVisibleSlot(mc, s.index);
