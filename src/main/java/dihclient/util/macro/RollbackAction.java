...
public final class RollbackAction implements MacroAction {
    ...
    public void execute(Minecraft mc) {
        if (mc == null || mc.isSameThread()) {
            DihClientMessaging.sendPrefixed("\u00a7cRollback must run from the macro executor.");
            return;
        }
        try {
            executeRollback(mc);
            ...
            DihClientMessaging.sendPrefixed("\u00a7cRollback failed: " + conciseMessage(t));
        }
    }
    ...
    public void executeRollback(Minecraft mc) throws Exception {
        ...
        if (server == null) {
            DihClientMessaging.sendPrefixed("\u00a7cRollback requires a singleplayer world or the LAN host.");
            return;
        }
    ...
    private TransferPlan createPlan(Minecraft mc) {
        if (mc.player == null || mc.gameMode == null || mc.player.containerMenu == null) {
            DihClientMessaging.sendPrefixed("\u00a7cRollback requires an open container.");
            return null;
        }

        AbstractContainerMenu menu = mc.player.containerMenu;
        if (menu == mc.player.inventoryMenu || menu.containerId == 0 || !menu.getCarried().isEmpty()) {
            DihClientMessaging.sendPrefixed("\u00a7cOpen a container and keep the cursor empty first.");
            return null;
        }
        ...
        if (selected.isEmpty()) {
            DihClientMessaging.sendPrefixed("\u00a7cRollback found no transferable container items.");
            return null;
        }
