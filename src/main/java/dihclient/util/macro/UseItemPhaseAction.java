...
public class UseItemPhaseAction implements MacroAction {
    ...
    public void sendUsePacket(Minecraft mc) {
        ...
        if (target.hasIdentity()) dihclient.util.DihInventoryHelper.selectHotbarItem(mc, target, mc.player.getInventory().getSelectedSlot());
    ...
    private void clickSlot(Minecraft mc, int slot, int button, ContainerInput input) {
        int handlerSlot = useCustomSlotMapping ? DihInventoryHelper.resolveConfiguredHandlerSlot(mc, slot) : slot;
        if (handlerSlot < 0) return;
        DihInventoryClickHelper.click(mc, handlerSlot, button, input);
    }
