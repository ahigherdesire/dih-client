...
import dihclient.gui.vanillaui.direct.DirectScrollList;
...
public class DihLANSyncOverlay extends DihOverlayBase {
    private static final Minecraft MC = Minecraft.getInstance();
    private static final CompactTheme PACKUI_THEME = new CompactTheme();
    private static DihLANSyncOverlay sharedOverlay;
    ...
    private DirectScrollList<DihMacro> sameMacroList = null;
    ...
    private DihChatField lanChatField;
    ...
    public DihLANSyncOverlay(Font textRenderer) {
    ...
    public static synchronized DihLANSyncOverlay getSharedOverlay(Font textRenderer) {
        if (sharedOverlay == null) {
            sharedOverlay = new DihLANSyncOverlay(textRenderer);
    ...
    public DihWindowLayout getBounds() {
        return new DihWindowLayout(panelX, panelY, PANEL_WIDTH, getPanelHeight(), visible, collapsed);
    }
    ...
    public void setConfigurationOnly(boolean configurationOnly) {
        ...
        DihLANSync.getInstance().setConfigurationOnly(configurationOnly);
    ...
    private int estimateTabContentHeight(DihLANSync sync) {
        switch (activeTab) {
            case 0: {
                int h = 4 + 12 * 4 + 4 + 14;
            ...
            case 2: {
                ...
                if (perUserMode) {
                    ...
                    int macros = DihMacroManager.get().getAll().size();
                    h += 14 + (macros == 0 ? 12 : macrolistViewportHeight(macros)) + 8;

                    if (selectedMacroName != null) h += 4 * 12;
                    h += BUTTON_HEIGHT + 6;
                }
                if (dihclient.util.macro.MacroExecutor.isVisibleRunning()) h += BUTTON_HEIGHT + 6;
                h += 1 + 6 + 14 + 14;
                return h;
            }
            case 3: {
    ...
    private boolean hasScrollableSessionContent(DihLANSync sync) {
    ...
    private void syncPerUserAssignments(DihLANSync sync) {
    ...
    private int estimatePerUserListContentHeight(DihLANSync sync) {
    ...
    private int estimateExpandedPeerPickerHeight(DihLANSync sync, String peer) {
        int contentHeight = 0;
        List<DihMacro> myMacroList = DihMacroManager.get().getAll();
        ...
        Map<String, Map<String, DihMacro>> allRemote = sync.getAllRemoteMacros();
        Map<String, DihMacro> peerMacroData = allRemote.getOrDefault(peer, java.util.Collections.emptyMap());
        java.util.Set<String> peerMacroKeys = new java.util.LinkedHashSet<>();
        for (DihMacro macro : peerMacroData.values()) {
        ...
        for (String key : peerMacroKeys) {
            if (myMacroKeys.contains(key)) {
                int diffs = countActionDifferences(DihMacroManager.get().get(key), peerMacroData.get(key));
                if (diffs == 0) {
                    shared.add(key);
                } else {
    ...
    public void render(GuiGraphicsExtractor ctx, int mouseX, int mouseY, float delta) {
        ...
        dihclient.gui.vanillaui.UiScissorStack.global().push(ctx,
            dihclient.gui.vanillaui.UiBounds.of(panelX, tabContentY, PANEL_WIDTH, actualTabContentH));
            switch (activeTab) {
    ...
    private void renderNotRunning(GuiGraphicsExtractor ctx, int mx, int my, int y) {
        ...
        drawUiText(ctx, "Not running", UiTone.LABEL, DihTheme.recolor(0xFFFF5555, DihTheme.Channel.DANGER), lx, y + 8);
        drawUiText(ctx, "LAN Sync is currently stopped", UiTone.MUTED, DihColors.textSecondary(), lx, y + 22);
    }
    ...
    private void renderNoSession(GuiGraphicsExtractor ctx, int mx, int my, int y) {
        ...
        drawUiText(ctx, "Host creates, others search & join", UiTone.MUTED, DihColors.textSecondary(), lx, y + 20);
        ...
        clickRegions.add(new ClickRegion(panelX + 8, btnY, halfW, BUTTON_HEIGHT, () -> {
            DihLANSync.getInstance().createSession();
        }));
        ...
        clickRegions.add(new ClickRegion(panelX + 12 + halfW, btnY, halfW, BUTTON_HEIGHT, () -> {
            DihLANSync.getInstance().startSearching();
        }));
    }

    private void renderSearching(GuiGraphicsExtractor ctx, int mx, int my, int y) {
        int secondsLeft = DihLANSync.getInstance().getSearchSecondsRemaining();
        ...
        clickRegions.add(new ClickRegion(panelX + 30, btnY, PANEL_WIDTH - 60, BUTTON_HEIGHT, () -> {
            DihLANSync.getInstance().cancelSearch();
        }));
    }

    private void ensureLanChatField(int panelHeight) {
        if (lanChatField == null) {
            lanChatField = new DihChatField(MC, textRenderer, panelX + CONTENT_PADDING, panelY, PANEL_WIDTH - (CONTENT_PADDING * 2), CHAT_FIELD_HEIGHT, true);
            lanChatField.setHistoryNavigationEnabled(true);
            lanChatField.setSubmitHandler(message -> DihLANSync.getInstance().sendChatMessage(message));
        }
        ...
        boolean enabled = !configurationOnly && DihLANSync.getInstance().isRunning() && DihLANSync.getInstance().isInSession();
    ...
    private void renderLanChatField(GuiGraphicsExtractor ctx, int mouseX, int mouseY) {
        ...
        DihText.draw(ctx, textRenderer, "Sync Send", DihText.Tone.MUTED, lanChatField.getX(), labelY, false);
    ...
    private void renderDashboard(GuiGraphicsExtractor ctx, int mx, int my, int y, int h) {
        ...
        drawUiText(ctx, "Session", UiTone.MUTED, DihColors.textSecondary(), lx, cy);
        ...
        drawUiText(ctx, "Role", UiTone.MUTED, DihColors.textSecondary(), lx, cy);
        drawUiText(ctx, role, UiTone.BODY, roleColor, lx + 50, cy);
        cy += 12;

        drawUiText(ctx, "Peers", UiTone.MUTED, DihColors.textSecondary(), lx, cy);
        ...
        cy += 4;
        DihLANSync.SyncState state = sync.getSyncState();
        String stateStr = state.name();
        int stateColor = getSyncStateColor(state);
        drawUiText(ctx, "Sync", UiTone.MUTED, DihColors.textSecondary(), lx, cy);
        drawUiText(ctx, stateStr, UiTone.BODY, stateColor, lx + 50, cy);
        cy += 14;

        DihLANSync.SpreadResult spread = sync.getLastSpreadResult();
        if (spread != null) {
            String spreadStr = spread.executionMethod == DihLANSync.ExecutionMethod.TICK
            ...
            drawUiText(ctx, "Spread", UiTone.MUTED, DihColors.textSecondary(), lx, cy);
            ...
            if (spread.lateClient != null && !spread.lateClient.isEmpty() && spread.getSpreadMs() > 0) {
                drawUiText(ctx, " (" + spread.lateClient + ")", UiTone.MUTED, DihColors.textDim(),
                    lx + 50 + UiText.width(textRenderer, spreadStr, PACKUI_THEME.fontFor(UiTone.BODY), spreadColor), cy);
            }
            cy += 12;
        }

        List<DihLANSync.SpreadResult> history = sync.getSpreadHistory();
        if (!history.isEmpty()) {
            drawUiText(ctx, "History", UiTone.MUTED, DihColors.textSecondary(), lx, cy);
            int dotX = lx + 52;
            for (DihLANSync.SpreadResult sr : history) {
        ...
        clickRegions.add(new ClickRegion(panelX + CONTENT_PADDING, btnY, PANEL_WIDTH - CONTENT_PADDING * 2, BUTTON_HEIGHT, () -> {
            DihLANSync.getInstance().leaveSession();
        }));
    }
    ...
    private void renderPeers(GuiGraphicsExtractor ctx, int mx, int my, int y, int h) {
        ...
        Map<String, DihLANSync.ClientInfo> clients = sync.getConnectedClients();
        List<DihLANSync.ClientInfo> sortedClients = new ArrayList<>(clients.values());
        sortedClients.sort((a, b) -> Boolean.compare(b.isHost, a.isHost));

        drawUiText(ctx, "Execution Method", UiTone.MUTED, DihColors.textSecondary(), lx, cy);
        cy += 12;

        int methodW = (rowWidth - 4) / 2;
        boolean instant = sync.getExecutionMethod() == DihLANSync.ExecutionMethod.INSTANT;
        boolean syncIdle = sync.getSyncState() == DihLANSync.SyncState.IDLE || sync.getSyncState() == DihLANSync.SyncState.DONE;
        ...
        if (canChangeMethod) {
            clickRegions.add(new ClickRegion(lx, cy, methodW, BUTTON_HEIGHT, () -> sync.setExecutionMethod(DihLANSync.ExecutionMethod.INSTANT)));
        }
        ...
        if (canChangeMethod) {
            clickRegions.add(new ClickRegion(lx + methodW + 4, cy, rowWidth - methodW - 4, BUTTON_HEIGHT, () -> sync.setExecutionMethod(DihLANSync.ExecutionMethod.TICK)));
        }
        cy += BUTTON_HEIGHT + 4;
        ...
        drawUiText(ctx, methodHint, UiTone.MUTED, DihColors.textDim(), lx, cy);
        cy += 14;

        for (DihLANSync.ClientInfo client : sortedClients) {
            ...
            CompactControlGlyphs.drawChevron(
                ...
                DihTheme.recolor(0xB8381418, DihTheme.Channel.OUTLINE),
                1.0f
            );

            DihLANSync.PeerStatus status = sync.getPeerStatus(client.username);
            int dotColor = status == DihLANSync.PeerStatus.HEALTHY ? 0xFF55FF55
                : status == DihLANSync.PeerStatus.STALE ? 0xFFFFAA00 : 0xFF888888;
            ...
            int rightReserve = 4;
            if (isSelf) rightReserve += UiText.width(textRenderer, " (YOU)", PACKUI_THEME.fontFor(UiTone.MUTED), DihColors.textSecondary()) + 2;
            ...
            String displayName = DihText.trimToWidth(textRenderer, client.username, nameMaxW, DihText.Tone.BODY);
            ...
            if (isSelf) {
                drawUiText(ctx, "(YOU)", UiTone.MUTED, DihColors.textSecondary(), badgeX, cy + 3);
                badgeX += UiText.width(textRenderer, "(YOU)", PACKUI_THEME.fontFor(UiTone.MUTED), DihColors.textSecondary()) + 4;
            }
            ...
            if (isSelected) {
                int detailH = 62;
                CompactSurfaces.insetPanel(ctx, lx + 4, cy, panelX + PANEL_WIDTH - CONTENT_PADDING - lx - 8, detailH, DihColors.secondary());

                int dx = lx + 10;
                int dy = cy + 4;

                drawUiText(ctx, "Status: " + status.name(), UiTone.BODY,
                    status == DihLANSync.PeerStatus.HEALTHY ? 0xFF55FF55 : 0xFFFFAA00, dx, dy);
                dy += 12;

                drawUiText(ctx, isSelf ? "(This is you)" : "Connected peer", UiTone.MUTED, DihColors.textDim(), dx, dy);
                dy += 13;

                drawUiText(ctx, "Exec delay", UiTone.MUTED, DihColors.textSecondary(), dx, dy);
                dy += 11;
    ...
    private void renderQueue(GuiGraphicsExtractor ctx, int mx, int my, int y, int h) {
        ...
        if (syncIdle) {
            clickRegions.add(new ClickRegion(lx + methodW + 4, cy, btnW - methodW - 4, BUTTON_HEIGHT, () -> sync.setExecutionMethod(DihLANSync.ExecutionMethod.TICK)));
        }
        cy += BUTTON_HEIGHT + 4;

        drawUiText(ctx, "My Queue", UiTone.LABEL, 0xFFFFFFFF, lx, cy);
        cy += 12;
        drawUiText(ctx, queueSize + " packets queued", UiTone.BODY, queueSize > 0 ? 0xFF55FF55 : DihColors.textDim(), lx, cy);
        cy += 16;
        ...
        if (syncIdle && !configurationOnly) {
            clickRegions.add(new ClickRegion(lx, cy, btnW, BUTTON_HEIGHT, () ->
                sync.setDelayPacketsSynchronized(!DihSharedState.get().shouldDelayGuiPackets())));
        }
        cy += BUTTON_HEIGHT + 8;
        ...
        if (!configurationOnly) clickRegions.add(new ClickRegion(lx, cy, btnW, BUTTON_HEIGHT, () -> {
            try {
                String queueData = DihClipboardHelper.serializeQueueToBase64(
                    DihSharedState.get().getDelayedPackets());
                sync.broadcastQueueSync(queueData);
                DihClientMessaging.sendPrefixed("§aQueue broadcasted to " + (sync.getConnectedCount() - 1) + " peers");
            } catch (Exception e) {
                DihClientMessaging.sendPrefixed("§cFailed to broadcast queue");
            }
        }));
        cy += BUTTON_HEIGHT + 8;
    ...
    private void renderExecute(GuiGraphicsExtractor ctx, int mx, int my, int y, int h) {
        ...
        boolean anyRunning = dihclient.util.macro.MacroExecutor.isVisibleRunning();
        ...
        if (perUserMode && !setBy.isEmpty()) {
            drawUiText(ctx, "Assigned by: " + setBy, UiTone.MUTED, DihColors.textDim(), lx, cy);
            cy += 12;
        }
    ...
    private int renderExecuteSameMacro(GuiGraphicsExtractor ctx, int mx, int my, int cy, int lx, int btnW, DihLANSync sync) {
        ...
        if (selectedMacroName != null) {
            String selectedLine = DihText.trimToWidth(textRenderer, "Selected: " + selectedMacroName, btnW, DihText.Tone.BODY);
            drawUiText(ctx, selectedLine, UiTone.BODY, 0xFFFFFFFF, lx, cy);
            cy += 12;

            if (!syncStatus.missingPeers.isEmpty()) {
                String missingLine = DihText.trimToWidth(
                    textRenderer,
                    "Missing on: " + formatNameSummary(syncStatus.missingPeers),
                    btnW,
                    DihText.Tone.BODY
                );
                drawUiText(ctx, missingLine, UiTone.BODY, DihTheme.recolor(0xFFFF6B6B, DihTheme.Channel.DANGER), lx, cy);
                cy += 12;
            }

            if (!syncStatus.differentPeers.isEmpty()) {
                String differentLine = DihText.trimToWidth(
                    textRenderer,
        ...
        int shareW = Math.min(112, Math.max(94, UiText.width(textRenderer, "Send To Peers", PACKUI_THEME.fontFor(UiTone.BODY), DihColors.textPrimary()) + 12));
    ...
    private int renderExecutePerUser(GuiGraphicsExtractor ctx, int mx, int my, int cy, int lx, int btnW, DihLANSync sync) {
        syncPerUserAssignments(sync);
        ...
        dihclient.gui.vanillaui.UiScissorStack.global().push(ctx,
            dihclient.gui.vanillaui.UiBounds.of(perUserListX + 1, perUserListY + 1,
        ...
        try {
            ...
            for (Map.Entry<String, String> entry : new ArrayList<>(perUserAssignments.entrySet())) {
                ...
                int assignColor = assigned.isEmpty() ? DihColors.textDim() : 0xFF55FF55;
                ...
                if (rowVisible) {
                    ...
                    if (rowBottom > rowTop) {
                        ...
                        if (textY < clipBottom - 2) {
                            String truncPeerLabel = DihText.trimToWidth(textRenderer, peerLabel, peerNameMaxW, DihText.Tone.BODY);
                            drawUiText(ctx, truncPeerLabel, UiTone.BODY, isMe ? DihColors.textSecondary() : 0xFFFFFFFF, lx + 10, textY);

                            String truncAssign = DihText.trimToWidth(textRenderer, assignLabel, maxAssignW, DihText.Tone.BODY);
                ...
                if (isExpanded) {
                    ...
                    if (isMe) {
                        ...
                        Map<String, DihMacro> peerMacroData = allRemote.getOrDefault(peer, java.util.Collections.emptyMap());
                        java.util.Set<String> peerMacroKeys = new java.util.LinkedHashSet<>();
                        for (DihMacro m : peerMacroData.values()) {
                            if (m == null) continue;
                            String key = MacroNames.key(m.name);
                        ...
                        for (String key : peerMacroKeys) {
                            ...
                            if (myMacroKeys.contains(key)) {
                                int diffs = countActionDifferences(
                                    DihMacroManager.get().get(key), peerMacroData.get(key));
                                if (diffs == 0) {
                                    shared.add(name);
                                } else {
                        ...
                        if (shared.isEmpty() && different.isEmpty() && peerOnly.isEmpty() && mineOnly.isEmpty()) {
                            if (cy + rowPitch > clipTop && cy < clipBottom) {
                                ...
                                if (rowBottom > rowTop) {
                                    ...
                                    int textY = Math.max(cy + 3, clipTop + 1);
                                    if (textY < clipBottom - 2) {
                                        drawUiText(ctx, "No macros available", UiTone.MUTED, DihColors.textDim(), detailLx, textY);
                                    }
                                }
                            }
                            cy += rowPitch;
        ...
        int shareW = Math.min(112, Math.max(96, UiText.width(textRenderer, "Send Missing", PACKUI_THEME.fontFor(UiTone.BODY), DihColors.textPrimary()) + 12));
        int executeX = lx + shareW + 4;
        int executeW = btnW - shareW - 4;
    ...
    private MacroSyncStatus analyzeSameMacroStatus(DihLANSync sync, String macroName) {
        ...
        DihMacro localMacro = DihMacroManager.get().get(macroName);
        if (localMacro == null) {
            return new MacroSyncStatus(missingPeers, differentPeers);
        }
        ...
        for (String peer : peers) {
            ...
            Map<String, DihMacro> peerMacros = allRemote.get(peer);
            if (peerMacros == null) continue;
            DihMacro remoteMacro = peerMacros.get(MacroNames.key(macroName));
    ...
    private AssignmentSyncStatus analyzeAssignmentStatus(DihLANSync sync) {
    ...
    private int renderPickerRowClipped(GuiGraphicsExtractor ctx, int mx, int my, int cy,
            ...
            String peerKey, DihLANSync sync, boolean allowImport,
        ...
        String displayName = DihText.trimToWidth(textRenderer, macroName, nameMaxW, DihText.Tone.BODY);
        int textY = Math.max(cy + 3, clipTop + 1);
        if (textY < clipBottom - 2) {
            UiText.draw(ctx, textRenderer, displayName, PACKUI_THEME.fontFor(UiTone.BODY), isCurrent ? DihColors.rowSelectedText() : 0xFFFFFFFF, tx, textY, false);
        }

        if (badge != null && textY < clipBottom - 2) {
            int badgeX = tx + UiText.width(textRenderer, displayName, PACKUI_THEME.fontFor(UiTone.BODY), isCurrent ? DihColors.rowSelectedText() : 0xFFFFFFFF) + 4;
        ...
        if (allowImport) {
            ...
            if (rowVisible) clickRegions.add(new ClickRegion(importX, cy, importW - 4, rowPitch, () -> {
                Map<String, Map<String, DihMacro>> allRemote = sync.getAllRemoteMacros();
                Map<String, DihMacro> peerMacros = allRemote.get(peerKey);
                DihMacro sourceMacro = peerMacros != null ? peerMacros.get(MacroNames.key(mName)) : null;
                if (sourceMacro == null) {
                    DihClientMessaging.sendPrefixed("§cMacro not found: " + mName);
                    return;
                }

                DihMacro imported = DihMacroManager.get().addImportedCopy(sourceMacro, sourceMacro.name);
                if (imported == null) {
                    DihClientMessaging.sendPrefixed("§cFailed to import macro: " + mName);
                    return;
                }

                DihClientMessaging.sendPrefixed("§aImported macro: " + imported.name);
            }));
        }
        return cy + rowPitch;
    }

    private static int countActionDifferences(DihMacro a, DihMacro b) {
        if (a == null || b == null) return -1;
        List<dihclient.util.macro.MacroAction> aActions = a.actions != null ? a.actions : java.util.Collections.emptyList();
        List<dihclient.util.macro.MacroAction> bActions = b.actions != null ? b.actions : java.util.Collections.emptyList();
        ...
        if (button == 0 && activeTab == 2 && !perUserMode && hasSameMacrolistViewport() && sameMacroList.contains(mouseX, mouseY)) {
            ...
            if (sameMacroList.mouseClicked(mouseX, mouseY, button)) {
                ...
                if (!sameMacroList.isScrollbarDragging()) {
                    DihMacro selected = sameMacroList.getSelectedItem();
                    if (selected != null) {
    ...
    private int getSyncStateColor(DihLANSync.SyncState state) {
        switch (state) {
            case IDLE: return DihColors.textDim();
            ...
            default: return DihColors.textSecondary();
        }
    }

    private String formatSyncState(DihLANSync.SyncState state, DihLANSync sync) {
        switch (state) {
    ...
    private int getSpreadColor(double spreadMs) {
        ...
        return DihTheme.recolor(0xFFFF5555, DihTheme.Channel.DANGER);
    }
