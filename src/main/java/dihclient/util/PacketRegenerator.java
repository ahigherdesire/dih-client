    ...
    public static <T extends Packet<?>> T regenerate(T original) {
        ...
        if (handler != null) {
            try {
                ...
                DihClientAddon.LOG.error("[Dih] Failed to regenerate {}: {}",
    ...
    private static void registerAllHandlers() {
        ...
        register(ServerboundContainerClickPacket.class, (packet, mc) -> {
            ...
            try {
                ...
                DihClientAddon.LOG.error("[Dih] ClickSlot regeneration failed: {}", e.getMessage());
                return packet;
            }
        });
        ...
        register(ServerboundCustomPayloadPacket.class, (packet, mc) -> {
            ...
            String channel = DihPayloadSupport.payloadChannel(payload);
            byte[] rawBytes = DihPayloadSupport.extractPayloadBytes(payload);

            if (channel == null || channel.isBlank()) {
                DihClientAddon.LOG.warn("[Dih] Cannot regenerate ServerboundCustomPayloadPacket - channel is blank");
                return null;
            }

            return DihPayloadSupport.createC2SPacket(channel, rawBytes);
        });
    }
