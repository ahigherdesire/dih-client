...
public final class MultiProxyVerifier {
    public record Result(boolean ok, long latencyMs, DihProxyType workingType) {
    ...
    public static Result verify(DihProxy proxy, String destHost, int destPort, int timeoutMs) {
        ...
        DihProxyType primary = proxy.type == DihProxyType.Socks4 ? DihProxyType.Socks4 : DihProxyType.Socks5;
        DihProxyType secondary = primary == DihProxyType.Socks4 ? DihProxyType.Socks5 : DihProxyType.Socks4;
        Result result = attempt(proxy, primary, host, destPort, timeout);
        if (result.ok()) return result;
        return attempt(proxy, secondary, host, destPort, timeout);
    }

    private static Result attempt(DihProxy proxy, DihProxyType type, String host, int port, int timeout) {
        ...
        try (Socket socket = new Socket()) {
            ...
            boolean ok = type == DihProxyType.Socks4
    ...
    private static boolean connectSocks5(OutputStream out, InputStream in, DihProxy proxy, String host, int port) throws IOException {
    ...
    private static boolean authSocks5(OutputStream out, InputStream in, DihProxy proxy) throws IOException {
    ...
    private static boolean connectSocks4(OutputStream out, InputStream in, DihProxy proxy, String host, int port) throws IOException {
        byte[] user = (proxy.username == null ? "" : proxy.username).getBytes(StandardCharsets.UTF_8);
