...
import dihclient.util.DihSharedState.QueuedPacket;
...
public class DihQueueEditorOverlay extends DihOverlayBase {
    ...
    public DihQueueEditorOverlay(Font textRenderer) {
    ...
    public boolean isCaptureMode() {
        return DihSharedState.get().isCaptureMode();
    }
    ...
    private int getContentStartOffset() {
        int toolbarRows = DihLANSync.getInstance().isInSession() ? 4 : 3;
    ...
    private void maybeAutoReorderDelayedQueue(DihSharedState shared) {
    ...
    public void render(GuiGraphicsExtractor context, int mouseX, int mouseY, float delta) {
        ...
        DihWindowLayout clamped = clampToScreen(
            this,
            new DihWindowLayout(panelX, panelY, PANEL_WIDTH, getPanelHeight(cachedDisplayQueue), visible, collapsed)
        );
        panelX = clamped.x;
        panelY = clamped.y;
        PANEL_WIDTH = clamped.width;
        ...
        bx = drawAutoToolbarButton(context, bx, row1Y, toolbarButtonHeight(), "Copy", mouseX, mouseY,
            ...
            () -> {
                DihClipboardHelper.copyToClipboard(getCurrentQueue());
                DihNotifications.copied("Queue copied.");
            }) + gap;
        bx = drawAutoToolbarButton(context, bx, row1Y, toolbarButtonHeight(), "Paste", mouseX, mouseY,
            "Replace queue from clipboard",
            () -> {
                if (DihSharedState.get().getStaggeredQueue().isEmpty()) {
                    DihSharedState.QueuedPacket.resetIdCounter();
                }
                List<QueuedPacket> newQueue = DihClipboardHelper.pasteFromClipboard();
                if (newQueue != null) {
                    DihSharedState.get().setDelayedPackets(newQueue);
                    clearSelection();
                    scrollOffset = 0;
                    DihClientMessaging.sendPrefixed("§aQueue pasted!");
                }
            }) + gap;
        bx = drawAutoToolbarButton(context, bx, row1Y, toolbarButtonHeight(), "Dupe", mouseX, mouseY,
            "Duplicate all packets in the queue",
            () -> {
                List<QueuedPacket> currentQueue = DihSharedState.get().getDelayedPackets();
                if (!currentQueue.isEmpty()) {
                    ...
                    DihSharedState.get().setDelayedPackets(duplicatedQueue);
                    clearSelection();
                    DihClientMessaging.sendPrefixed("§aDuplicated " + currentQueue.size() + " packet(s).");
                }
            }) + gap;
        bx = drawAutoToolbarButton(context, bx, row1Y, toolbarButtonHeight(), "Ins", mouseX, mouseY,
            ...
            () -> {
                List<QueuedPacket> newPackets = DihClipboardHelper.pasteFromClipboard();
                if (newPackets != null && !newPackets.isEmpty()) {
                    List<QueuedPacket> combinedQueue = new ArrayList<>(DihSharedState.get().getDelayedPackets());
                    combinedQueue.addAll(newPackets);
                    DihSharedState.get().setDelayedPackets(combinedQueue);
                    clearSelection();
                    DihClientMessaging.sendPrefixed("§aInserted " + newPackets.size() + " packet(s).");
                }
            }) + gap;
        ...
        drawFixedToolbarButton(context, bx, row1Y, replayWidth, toolbarButtonHeight(), "Replay", CompactOverlayButton.Variant.GHOST, true, mouseX, mouseY,
            ...
            () -> {
                if (DihSharedState.get().restoreLastFlushedQueue()) {
                    clearSelection();
                    scrollOffset = 0;
                    DihClientMessaging.sendPrefixed("§aRestored - packets will regenerate on send");
                } else {
                    DihClientMessaging.sendPrefixed("§cNo history");
                }
            });

        boolean flushOnDelay = DihModule.get().shouldFlushQueueOnDelayDisable();
        boolean captureAsExact = DihModule.get().isCaptureAsExact();
        ...
        drawFixedToolbarStateButton(context, leftX, row2Y, flushWidth, toolbarButtonHeight(), "FLUSH ON DELAY", flushOnDelay, mouseX, mouseY,
            ...
            () -> {
                ...
                DihClientMessaging.sendPrefixed(newValue
                    ? "\u00a7aFlush on Delay ON"
                    : "\u00a7eFlush on Delay OFF");
            });
        drawFixedToolbarButton(context, leftX + flushWidth + gap, row2Y, captureModeBtnWidth, toolbarButtonHeight(),
            ...
            () -> {
                ...
                DihSharedState.ReplayMode bulkMode = nowExact
                    ? DihSharedState.ReplayMode.EXACT
                    : DihSharedState.ReplayMode.REGENERATE;
                int updated = 0;
                for (DihSharedState.QueuedPacket qp : DihSharedState.get().getDelayedPackets()) {
                    if (qp.getReplayMode() != bulkMode) {
                        qp.setReplayMode(bulkMode);
                        updated++;
                    }
                }
                DihClientMessaging.sendPrefixed(nowExact
        ...
        drawFixedToolbarButton(context, leftX + flushWidth + gap + captureModeBtnWidth + gap, row2Y, clearWidth, toolbarButtonHeight(), "Clear", CompactOverlayButton.Variant.DANGER, true, mouseX, mouseY,
            ...
            () -> {
                int cleared = DihSharedState.get().clearQueuedPackets();
                clearSelection();
                scrollOffset = 0;
                DihModule.get().notifyClearQueuedPacketsUiResult(cleared);
            });

        boolean captureModeOn = DihSharedState.get().isCaptureMode();
        String delayModeLabel = DihSharedState.get().getDelayMode() == DihSharedState.DelayMode.TICKS ? "Ticks" : "Ms";
        ...
        drawFixedToolbarButton(context, leftX, row3Y, modeWidth, toolbarButtonHeight(), delayModeLabel, CompactOverlayButton.Variant.GHOST, true, mouseX, mouseY,
            ...
            () -> {
                DihSharedState.get().toggleDelayMode();
                String newMode = DihSharedState.get().getDelayMode() == DihSharedState.DelayMode.TICKS ? "Ticks" : "Ms";
                DihClientMessaging.sendPrefixed("\u00a7aDelay mode: " + newMode);
            });
        drawFixedToolbarStateButton(context, leftX + modeWidth + gap, row3Y, captureWidth, toolbarButtonHeight(), "Capture", captureModeOn, mouseX, mouseY,
            ...
            () -> {
                boolean newCapture = !DihSharedState.get().isCaptureMode();
                DihSharedState.get().setCaptureMode(newCapture);
                DihClientMessaging.sendPrefixed(newCapture ? "\u00a7aCapture Real Delays ON" : "\u00a7eCapture Real Delays OFF");
            });
        ...
        drawFixedToolbarButton(context, rowRight - sendWidth, row3Y, sendWidth, toolbarButtonHeight(), configurationOnly ? "In Game" : "Send Q",
            ...
            () -> {
                ...
                if (MC.getConnection() != null) {
                    int count = DihSharedState.get().flushDelayedPackets(MC.getConnection());
                    DihModule.get().notifyFlushQueuedPacketsUiResult(count);
                }
            });

        boolean inLanSession = DihLANSync.getInstance().isInSession();
        if (inLanSession) {
            ...
            drawFixedToolbarButton(context, leftX + syncWidth + gap, row4Y, syncExecWidth, toolbarButtonHeight(), "Sync Exec", CompactOverlayButton.Variant.SUCCESS, !configurationOnly, mouseX, mouseY,
                ...
                () -> DihLANSync.getInstance().sendQueuedPackets());
        }

        if (cachedDisplayQueue.isEmpty()) {
            DihText.draw(context, textRenderer, "Queue empty", DihText.Tone.MUTED, panelX + 10, getContentStartY(), false);
        } else {
            int visibleHeight = getlistViewportHeight(panelHeight);
            ...
            dihclient.gui.vanillaui.UiScissorStack.global().push(context,
                dihclient.gui.vanillaui.UiBounds.of(listX + 1, clipTop, Math.max(0, listWidth - 2), Math.max(0, clipBottom - clipTop)));
            ...
            for (int i = 0; i < cachedDisplayQueue.size(); i++) {
                ...
                int textColor = DihColors.textPrimary();
                if (grouped) {
                ...
                String label = "#" + packet.getId() + " " + DihPacketNamer.getFriendlyName(packet.packet);
                ...
                UiText.draw(context, textRenderer, trimmed, theme.fontFor(UiTone.BODY), selected ? DihColors.rowSelectedText() : textColor, rowX + 7, rowTextY, false);
    ...
    public boolean mouseScrolled(double mouseX, double mouseY, double amount) {
        ...
        List<QueuedPacket> displayQueue = cachedDisplayQueue.isEmpty() ? getCurrentQueue() : cachedDisplayQueue;
        int panelHeight = clampToScreen(this, new DihWindowLayout(panelX, panelY, PANEL_WIDTH, getPanelHeight(displayQueue), visible, collapsed)).height;
        int visibleHeight = getlistViewportHeight(panelHeight);
    ...
    private void handleSyncButton() {
        ...
        if (!sync.isInSession()) {
            DihClientMessaging.sendPrefixed("\u00a7cNot in LAN session");
            return;
        }

        List<QueuedPacket> queue = getCurrentQueue();
        if (queue.isEmpty()) {
            DihClientMessaging.sendPrefixed("\u00a7cQueue is empty");
            return;
        }

        String queueData = DihClipboardHelper.serializeQueueToBase64(queue);
        if (queueData == null) {
            DihClientMessaging.sendPrefixed("\u00a7cFailed to serialize queue");
            return;
        }

        sync.broadcastQueueSync(queueData);
        DihClientMessaging.sendPrefixed("\u00a7aQueue sent to " + sync.getConnectedClientCount() + " clients");
    }
    ...
    private void handleIncrementalDelay(int increment) {
        ...
        if (selectedPacketId != -1) {
            for (QueuedPacket packet : queue) {
                if (packet.getId() == selectedPacketId) {
                    ...
                    String modeStr = DihSharedState.get().getQueueDisplayDelayMode() == DihSharedState.DelayMode.TICKS ? "ticks" : "ms";
                    DihClientMessaging.sendPrefixed("\u00a7aPacket #" + packet.getId() + " delay: " + packet.getDelay() + " " + modeStr);
                    return;
                }
            }
            clearSelection();
            DihClientMessaging.sendPrefixed("\u00a7cSelected packet not found");
            return;
        }
        ...
        DihClientMessaging.sendPrefixed("\u00a7aApplied +" + increment + " incremental delays");
    }
