...
public class DihCustomFilterPresetOverlay extends DihOverlayBase {
    ...
    private List<DihPresetManager.PresetEntry> cachedBuiltInEntries = List.of();
    private List<DihPresetManager.PresetEntry> cachedUserEntries = List.of();
    ...
    public DihCustomFilterPresetOverlay(Font textRenderer) {
    ...
    public String getOverlayId() {
        return "dih-custom-filter-presets";
    }
    ...
    public void render(GuiGraphicsExtractor context, int mouseX, int mouseY, float delta) {
        ...
        buttons.clear();
        DihWindowLayout clamped = clampToScreen(this, new DihWindowLayout(panelX, panelY, panelWidth, panelHeight, visible, collapsed));
        panelX = clamped.x;
        panelY = clamped.y;
        panelWidth = clamped.width;
        panelHeight = clamped.height;
        DihWindowLayout bounds = new DihWindowLayout(panelX, panelY, panelWidth, panelHeight, visible, collapsed);
        ...
        try {
            ...
            DihText.draw(context, textRenderer,
                "C2S " + module.getC2SPackets().size() + " | S2C " + module.getS2CPackets().size(),
                DihText.Tone.MUTED, x, y, false);
            y += infoLineHeight();
    ...
    private int renderPresetSection(GuiGraphicsExtractor context, int mouseX, int mouseY, float delta, int x, int y, int width, int sectionHeight) {
                ...
                listLayout.forEachVisibleRow(rows.size(), (index, rowY) -> {
                    ...
                    if (row.header) {
                        ...
                        int textColor = selected
                            ? DihColors.rowSelectedText()
                            : (row.entry.builtIn() ? 0xFFB7D4FF : DihColors.textLight());
                        String prefix = row.entry.builtIn() ? "[DEV] " : "[USR] ";
                        String name = DihText.trimToWidth(textRenderer, prefix + row.entry.name(), contentWidth - 70, DihText.Tone.BODY);
                        DihText.draw(context, textRenderer, name, textColor, x + 5, rowTextY, false);

                        String counts = row.entry.c2sCount() + " / " + row.entry.s2cCount();
                        int countsWidth = DihText.width(textRenderer, counts, DihText.Tone.BODY);
                        DihText.draw(context, textRenderer, counts, textColor, x + contentWidth - countsWidth - 1, rowTextY, false);
                    }
        ...
        DihText.draw(context, textRenderer, footer, DihText.Tone.MUTED, x + 4, y + listHeight + footerGap(), false);
        return y + listHeight + footerHeight() + footerGap();
    }

    private List<PresetRow> getCachedPresetRows() {
        DihPresetManager manager = DihPresetManager.get();
        List<DihPresetManager.PresetEntry> builtInEntries = manager.getBuiltInPresetEntries();
        List<DihPresetManager.PresetEntry> userEntries = manager.getUserPresetEntries();
        ...
        for (DihPresetManager.PresetEntry entry : builtInEntries) {
        ...
        if (userEntries.isEmpty()) {
            ...
            for (DihPresetManager.PresetEntry entry : userEntries) {
                rows.add(PresetRow.entry(entry));
            }
        }
    ...
    private void saveNamedPreset() {
        ...
        if (name == null) {
            DihClientMessaging.sendPrefixed("Enter a preset name first.");
            return;
        }
        if (DihPresetManager.get().savePreset(name)) {
    ...
    private void loadSelectedPreset() {
        ...
        if (presetName == null) {
            DihClientMessaging.sendPrefixed("Select a preset first.");
            return;
        }
        if (DihPresetManager.get().loadPreset(presetName)) {
            selectedPresetName = DihPresetManager.get().getPresetEntry(presetName) != null
                ? DihPresetManager.get().getPresetEntry(presetName).name()
                : presetName;
    ...
    private void overwriteSelectedPreset() {
        String presetName = resolveTargetPresetName();
        if (presetName == null) {
            DihClientMessaging.sendPrefixed("Select a user preset first.");
            return;
        }
        DihPresetManager.PresetEntry entry = DihPresetManager.get().getPresetEntry(presetName);
        if (entry == null || entry.builtIn()) {
            DihClientMessaging.sendPrefixed("Select a user preset to overwrite.");
            return;
        }
        if (DihPresetManager.get().overwriteUserPreset(entry.name())) {
    ...
    private void deleteSelectedPreset() {
        ...
        DihPresetManager.PresetEntry entry = DihPresetManager.get().getPresetEntry(presetName);
        if (entry == null || entry.builtIn()) {
            DihClientMessaging.sendPrefixed("Developer presets cannot be deleted.");
            return;
        }
        if (DihPresetManager.get().deleteUserPreset(entry.name())) {
    ...
    private void resetDefaults() {
        ...
        DihClientMessaging.sendPrefixed("Reset current filter to default packet lists.");
    }
    ...
    private String resolveTargetPresetName() {
        if (selectedPresetName != null) {
            DihPresetManager.PresetEntry selectedEntry = DihPresetManager.get().getPresetEntry(selectedPresetName);
            if (selectedEntry != null) return selectedEntry.name();
        }

        String typedName = sanitizeNameField();
        if (typedName == null) return null;
        DihPresetManager.PresetEntry typedEntry = DihPresetManager.get().getPresetEntry(typedName);
        return typedEntry != null ? typedEntry.name() : null;
    }

    private DihPresetManager.PresetEntry getSelectedPresetEntry() {
        if (selectedPresetName == null) return null;
        return DihPresetManager.get().getPresetEntry(selectedPresetName);
    }

    private boolean canSaveNamed() {
        String name = sanitizeNameField();
        return name != null && !DihPresetManager.get().isReservedPresetName(name);
    }
    ...
    private boolean canOverwriteSelected() {
        DihPresetManager.PresetEntry entry = getSelectedPresetEntry();
    ...
    private final class PresetListState {
        ...
        private PresetRow getRowAt(double mouseY) {
            int idx = (int) (((mouseY - y - 2) + scroll.visualOffsetInt()) / DihCustomFilterPresetOverlay.this.presetRowStep());
    ...
    private static final class PresetRow {
        ...
        private final DihPresetManager.PresetEntry entry;

        private PresetRow(boolean header, boolean note, String label, DihPresetManager.PresetEntry entry) {
            this.header = header;
            this.note = note;
            this.label = label;
            this.entry = entry;
        ...
        private static PresetRow entry(DihPresetManager.PresetEntry entry) {
