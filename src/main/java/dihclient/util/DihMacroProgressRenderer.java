...
import dihclient.util.macro.MacroStepPalette;
...
public final class DihMacroProgressRenderer {
    ...
    private DihMacroProgressRenderer() {
    }
    ...
    private static int totalFor(MacroExecutor.MacroRunSnapshot run, DihMacro macro) {
    ...
    private static boolean shouldRebuildCache(boolean runsMatch, int current, int total, int scroll, int visibleRows, int panelWidth, long now) {
        boolean layoutChanged = !runsMatch
            ...
            || cachedThemeGeneration != DihTheme.generation();
        if (layoutChanged) {
            return true;
        }
    ...
    private static int stepNowColor() {
        return DihTheme.recolor(STEP_NOW_COLOR, DihTheme.Channel.ACCENT);
    }
