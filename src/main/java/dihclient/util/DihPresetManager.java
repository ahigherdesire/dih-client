...
public class DihPresetManager {
    private static final File PRESETS_FOLDER = new File(DihClientAddon.FOLDER, "presets");
    private static final DihPresetManager INSTANCE = new DihPresetManager();
    ...
    public record PresetEntry(DihPacketPreset preset, boolean builtIn) {
        public String name() {
    ...
    private DihPresetManager() {
        if (!PRESETS_FOLDER.exists()) {
            PRESETS_FOLDER.mkdirs();
        }
    }

    public static DihPresetManager get() {
        return INSTANCE;
    }
    ...
    public boolean savePreset(String name) {
        ...
        if (sanitized == null) {
            DihClientMessaging.sendPrefixed("§ePreset name cannot be empty.");
            return false;
        }
        if (isReservedPresetName(sanitized)) {
            DihClientMessaging.sendPrefixed("§cThat preset name is reserved for developer templates.");
            return false;
        }
    ...
    public String savePacketCancellerPreset(Set<String> c2sPackets, Set<String> s2cPackets) {
        String name = nextAvailableUserPresetName();
        savePresetObject(new DihPacketPreset(name,
    ...
    public boolean overwriteUserPreset(String name) {
        String sanitized = sanitizePresetName(name);
        if (sanitized == null) {
            DihClientMessaging.sendPrefixed("§eSelect a user preset first.");
            return false;
        }
        if (isReservedPresetName(sanitized)) {
            DihClientMessaging.sendPrefixed("§cDeveloper presets cannot be overwritten.");
            return false;
        }

        File file = new File(PRESETS_FOLDER, sanitized + ".json");
        if (!file.exists()) {
            DihClientMessaging.sendPrefixed("§cUser preset not found: " + sanitized);
            return false;
        }
    ...
    public boolean loadPreset(String name) {
        ...
        if (entry == null) {
            DihClientMessaging.sendPrefixed("§cPreset not found: " + name);
            return false;
        }

        applyPreset(entry.preset());
        DihClientMessaging.sendPrefixed("§aLoaded preset: " + entry.name());
        return true;
    }
    ...
    public boolean deleteUserPreset(String name) {
        ...
        if (isReservedPresetName(sanitized)) {
            DihClientMessaging.sendPrefixed("§cDeveloper presets cannot be deleted.");
            return false;
        }
        ...
        if (!file.delete()) {
            DihClientMessaging.sendPrefixed("§cFailed to delete preset: " + sanitized);
            return false;
        }

        invalidateUserPresetCache();
        DihClientMessaging.sendPrefixed("§aDeleted preset: " + sanitized);
        return true;
    }
    ...
    public boolean importSharedPreset(String presetName, String presetJson, String sender) {
        ...
        try {
            DihPacketPreset preset = gson.fromJson(presetJson, DihPacketPreset.class);
            if (preset == null) return false;
            preset.name = createUniquePresetName(sanitized);
            savePresetObject(preset, false);
            invalidateUserPresetCache();
            DihClientMessaging.sendPrefixed("§aReceived preset from " + sender + ": " + preset.name);
            return true;
        } catch (Exception e) {
            DihClientAddon.LOG.error("Failed to import shared preset", e);
            DihClientMessaging.sendPrefixed("§cFailed to import preset: " + e.getMessage());
            return false;
        }
    }
    ...
    public List<PresetEntry> getUserPresetEntries() {
        ...
        for (File file : files) {
            try (FileReader reader = new FileReader(file)) {
                DihPacketPreset preset = gson.fromJson(reader, DihPacketPreset.class);
                if (preset == null) continue;
                if (preset.name == null || preset.name.isBlank()) {
                    preset.name = file.getName().replace(".json", "");
                }
                if (isReservedPresetName(preset.name)) {
                    DihClientAddon.LOG.warn("Ignoring user preset with reserved built-in name: {}", preset.name);
                    continue;
                }
                ...
                DihClientAddon.LOG.error("Failed to read preset file {}", file.getName(), e);
            }
        }
    ...
    private DihPacketPreset captureCurrentPreset(String name) {
        ...
        return new DihPacketPreset(name, c2s, s2c);
    }

    private DihPacketPreset presetFromClasses(String name,
                                                   Collection<Class<? extends Packet<?>>> c2sPackets,
                                                   Collection<Class<? extends Packet<?>>> s2cPackets) {
        return new DihPacketPreset(name, encodePacketNames(c2sPackets), encodePacketNames(s2cPackets));
    }

    private DihPacketPreset presetFromPacketNames(String name, Set<String> c2sNames, Set<String> s2cNames) {
        return new DihPacketPreset(
            name,
    ...
    private Set<Class<? extends Packet<?>>> resolvePacketNames(Collection<String> names, boolean c2s) {
        ...
        Set<Class<? extends Packet<?>>> pool = c2s ? DihPacketRegistry.getC2SPackets() : DihPacketRegistry.getS2CPackets();
        for (String packetName : names) {
            ...
            Class<? extends Packet<?>> packetClass = DihPacketRegistry.getPacket(resolvedName);
            if (packetClass == null) {
                packetClass = DihPacketRegistry.getPacket(DihPacketNamer.getFriendlyName(resolvedName));
            }

            if (packetClass != null && pool.contains(packetClass)) {
                resolved.add(packetClass);
            } else {
                DihClientAddon.LOG.warn("Unknown packet name in preset: {}", packetName);
            }
        }
        return resolved;
    }

    private void savePresetObject(DihPacketPreset preset, boolean notify) {
        String safeName = sanitizePresetName(preset.name);
        if (safeName == null) {
            DihClientMessaging.sendPrefixed("§cInvalid preset name.");
            return;
        }
        File file = new File(PRESETS_FOLDER, safeName + ".json");
        try (FileWriter writer = new FileWriter(file)) {
            gson.toJson(preset, writer);
            if (notify) DihClientMessaging.sendPrefixed("§aSaved preset: " + preset.name);
        } catch (IOException e) {
            DihClientAddon.LOG.error("Failed to save preset", e);
            DihClientMessaging.sendPrefixed("§cFailed to save preset: " + e.getMessage());
        }
    }

    private void applyPreset(DihPacketPreset preset) {
        DihModule module = DihModule.get();
