...
public class DihProxy {
    ...
    public String name = "";
    public DihProxyType type = DihProxyType.Socks5;
    ...
    public void applyGeoResult(DihProxyGeoLookup.GeoResult result) {
        if (result == null) {
    ...
    private TypeProbeResult checkType(DihProxyType checkType, int timeoutMs) {
        try {
            Instant before = Instant.now();
            boolean alive = checkType == DihProxyType.Socks4 ? isSocks4(timeoutMs) : isSocks5(timeoutMs);
            if (alive) {
    ...
    public CompoundTag toTag() {
        ...
        tag.putString("type", type == null ? DihProxyType.Socks5.name() : type.name());
    ...
    public DihProxy fromTag(CompoundTag tag) {
        ...
        String typeName = tag.getStringOr("type", DihProxyType.Socks5.name());
        try {
            type = DihProxyType.valueOf(typeName);
        } catch (IllegalArgumentException e) {
            type = typeName.toLowerCase(Locale.ROOT).contains("4") ? DihProxyType.Socks4 : DihProxyType.Socks5;
        }
    ...
    public boolean equals(Object obj) {
        ...
        if (!(obj instanceof DihProxy proxy)) return false;
