...
import dihclient.gui.vanillaui.direct.DirectHudPanelRenderer;
...
public final class DihQueueRenderer {
    ...
    private static void ensureCache(Font textRenderer, int panelWidth, int maxLines) {
        ...
        String modeStr = shared.getQueueDisplayDelayMode() == DihSharedState.DelayMode.MS ? "ms" : "t";
    ...
    private static void rebuildCache(Font textRenderer, DihSharedState.QueueRenderSnapshot snapshot, boolean isSending, String modeStr, int maxLines, int panelWidth, int queueRevision, long now) {
        List<DihSharedState.QueuedPacket> packets = snapshot.packets();
        ...
        if (packets.isEmpty()) {
            ...
            for (int i = 0; i < visibleCount; i++) {
                ...
                String simpleName = DihPacketNamer.getFriendlyName(qp.packet);
