...
import dihclient.gui.mm.MatchmakingPanel;
...
public final class DihMatchmakingOverlay extends DihOverlayBase implements MatchmakingPanel.Host {
    private static final String OVERLAY_ID = "dih-matchmaking";
    ...
    public DihMatchmakingOverlay(Font font) {
    ...
    public void toggle() {
        ...
        setVisible(!visible);
        if (visible) DihOverlayManager.get().bringToFront(this);
    }
    ...
    public void render(GuiGraphicsExtractor ctx, int mx, int my, float delta) {
        ...
        if (!collapsed) {
            ...
            int maxH = Math.min(DihUiScale.getVirtualScreenHeight() - 16, MAX_H);
    ...
    private void renderMainMenu(GuiGraphicsExtractor ctx, int mx, int my, float delta) {
        ...
        dihclient.gui.vanillaui.components.Card.render(
            dihclient.gui.vanillaui.UiContexts.overlay(ctx, font, mx, my), UiBounds.of(px - 2, py - 2, pw + 4, ph + 4));
