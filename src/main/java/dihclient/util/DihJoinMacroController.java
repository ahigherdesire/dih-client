...
public final class DihJoinMacroController {
    public enum Timing {
    ...
    private static volatile DihMacro armedRemainder;
    ...
    private DihJoinMacroController() {
    }

    public static Timing timing() {
        return Timing.fromConfig(DihConfig.getGlobal().joinMacroTiming);
    }
    ...
    public static TriggerJoin triggerJoin() {
        return normalizeTriggerJoin(TriggerJoin.fromConfig(DihConfig.getGlobal().joinMacroTriggerJoin), keepEnabled());
    }
    ...
    public static boolean keepEnabled() {
        return DihConfig.getGlobal().joinMacroKeepEnabled;
    }
    ...
    public static String selectedMacroName() {
        String name = DihConfig.getGlobal().joinMacroName;
    ...
    public static void onConfigurationInit(net.minecraft.client.multiplayer.ClientConfigurationPacketListenerImpl listener) {
        ...
        DihMacro selected = DihMacroManager.get().get(selectedMacroName());
        if (selected == null) return;
        DihMacro copy = selected.deepCopy();
        int prefix = 0;
        boolean sawCustomMenu = false;
        while (prefix < copy.actions.size()) {
            dihclient.util.macro.MacroAction action = copy.actions.get(prefix);
        ...
        java.util.List<dihclient.util.macro.MacroAction> leading =
            java.util.List.copyOf(copy.actions.subList(0, prefix));
        Thread worker = new Thread(() -> runPreJoinActions(leading), "DIH-Join-CustomMenu");
        worker.setDaemon(true);
        preJoinThread = worker;
        worker.start();
    }

    private static boolean isPreJoinWait(dihclient.util.macro.MacroAction action) {
        return action instanceof dihclient.util.macro.WaitForGuiAction
            || action instanceof dihclient.util.macro.WaitGuiTypeAction
            || action instanceof dihclient.util.macro.DelayAction;
    }
    ...
    private static void runPreJoinActions(java.util.List<dihclient.util.macro.MacroAction> actions) {
        for (dihclient.util.macro.MacroAction macroAction : actions) {
            ...
            while (System.currentTimeMillis() <= deadline && !Thread.currentThread().isInterrupted()) {
                ...
                prepared = CustomMenuActionSupport.prepare(action, snapshot, DihJoinMacroController::resolveFormTemplate);
    ...
    private static void waitPreJoin(dihclient.util.macro.MacroAction action) {
        long timeoutMs;
        if (action instanceof dihclient.util.macro.WaitForGuiAction w) {
            timeoutMs = w.timeoutMs > 0 ? w.timeoutMs : 30_000L;
        } else if (action instanceof dihclient.util.macro.WaitGuiTypeAction w) {
            timeoutMs = w.timeoutMs > 0 ? w.timeoutMs : 30_000L;
        } else if (action instanceof dihclient.util.macro.DelayAction d) {
    ...
    private static void skipPreJoin(String reason) {
        DihNotifications.warning(reason == null || reason.isBlank() ? "Join custom menu skipped" : reason);
    }
    ...
    public static Map<String, String> openFormValues() {
        ...
        Map<String, String> stored = DihConfig.getGlobal().joinMacroFormValues;
        if (stored == null) return result;
        stored.forEach((name, encoded) -> {
            try {
                byte[] plain = dihclient.util.mm.crypto.AtRestSeal.unseal(Base64.getDecoder().decode(encoded));
    ...
    public static boolean setFormValues(Map<String, String> values) {
        ...
        if (values != null) {
            for (Map.Entry<String, String> entry : values.entrySet()) {
                String name = dihclient.util.multi.MultiProfile.normalizeSecretName(entry.getKey());
                if (name.isEmpty()) continue;
                byte[] blob = dihclient.util.mm.crypto.AtRestSeal.seal(
    ...
    private static void executeOnce(Timing timing) {
        ...
        DihMacro macro = armedRemainder != null ? armedRemainder : DihMacroManager.get().get(macroName);
        if (macro == null) {
            DihNotifications.warning("Join macro missing: " + macroName);
            setSelectedMacro("");
            return;
        }
        ...
        Runnable run = () -> {
            try {
                ...
                DihNotifications.show("Join macro: " + macro.name, 0xFF35D873);
            } catch (Throwable t) {
                DihNotifications.error("Join macro failed.");
            }
        };
