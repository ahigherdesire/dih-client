...
public class DihPacketSelectorOverlay extends DihOverlayBase {
    ...
    private final SearchableSelector<Class<? extends Packet<?>>> selector =
        new SearchableSelector<>(packetClass -> packetSearchKeys.computeIfAbsent(packetClass, DihPacketSelectorOverlay::searchKey));
    ...
    public DihPacketSelectorOverlay(Font textRenderer) {
        this.textRenderer = textRenderer;
        sortedC2sPackets.addAll(DihPacketRegistry.getC2SPackets());
        sortedC2sPackets.sort(Comparator.comparing(DihPacketNamer::getFriendlyName, String.CASE_INSENSITIVE_ORDER));
        sortedS2cPackets.addAll(DihPacketRegistry.getS2CPackets());
        sortedS2cPackets.sort(Comparator.comparing(DihPacketNamer::getFriendlyName, String.CASE_INSENSITIVE_ORDER));
        ...
        this.allPackets.sort(Comparator.comparing(DihPacketNamer::getFriendlyName, String.CASE_INSENSITIVE_ORDER));
    ...
    private void finishOpen() {
        ...
        DihOverlayManager.get().bringToFrontParent(this);
    }
    ...
    private void renderPacketRowSimple(GuiGraphicsExtractor context, Class<? extends Packet<?>> packetClass, int x, int y, int width, int index) {
        ...
        int rowColor = selected ? DihColors.packetRowSelectedBg(false) : DihColors.packetRowBg(c2s, index, false);
        int color = selected ? DihColors.packetRowSelectedText() : DihColors.packetRowText(c2s, index);
        CompactSurfaces.tintedRow(context, x, y, width, rowHeight(), rowColor);

        String name = DihPacketNamer.getFriendlyName(packetClass);
        ...
        CompactSurfaces.divider(context, x + 4, y + rowHeight() - 1, width - 8, DihColors.packetRowDivider());
    }
    ...
    private static String searchKey(Class<? extends Packet<?>> packetClass) {
        ...
        return (packetClass.getSimpleName() + '\n' + DihPacketNamer.getFriendlyName(packetClass))
