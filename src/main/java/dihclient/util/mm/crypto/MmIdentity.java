package dihclient.util.mm.crypto;
...
public final class MmIdentity {
    private static final File FILE = new File(DihClientAddon.FOLDER, "mm_identity.dat");
    private static final File BAK = new File(DihClientAddon.FOLDER, "mm_identity.dat.bak");
    private static final File TMP = new File(DihClientAddon.FOLDER, "mm_identity.dat.tmp");

    private static final File KEY_FILE = new File(DihClientAddon.FOLDER, "mm_identity.key");
    private static final File KEY_TMP = new File(DihClientAddon.FOLDER, "mm_identity.key.tmp");
    ...
    private static MmIdentity load() {
        if (FILE.exists()) {
            ...
            if (backup != null) {
                DihClientAddon.LOG.warn("Matchmaking identity was unreadable; restored it from the backup copy");
                save(backup.identity);
                return backup.identity;
            }

            quarantineCorrupt();
            DihClientAddon.LOG.error("Matchmaking identity is corrupt or could not be decrypted, and no usable "
    ...
    private static void save(MmIdentity identity) {
        try {
            ...
            DihClientAddon.LOG.error("Failed to persist Matchmaking identity", t);
        }
    }
    ...
    private static Sealed sealBoth(byte[] edPkcs8, byte[] xPkcs8) {
        ...
        DihClientAddon.LOG.warn("Matchmaking identity stored WITHOUT encryption (no OS encryption and the key "
