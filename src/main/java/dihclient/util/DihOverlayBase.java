...
public abstract class DihOverlayBase extends DihWindow implements IDihOverlay {
    ...
    private DihContextMenu<?> contextMenu;

    protected DihOverlayBase() {
        this(null, 0, 0);
    }

    protected DihOverlayBase(String overlayId, int initialWidth, int initialHeight) {
    ...
    public void setBounds(DihWindowLayout bounds) {
        ...
        DihWindowLayout c = clampToScreen(this, bounds);
        panelX = c.x;
        panelY = c.y;
        panelWidth = c.width;
    ...
    protected void setContextMenu(DihContextMenu<?> menu) {
        this.contextMenu = menu;
    }

    protected DihContextMenu<?> contextMenu() {
        return contextMenu;
    }
    ...
    protected int hoverBlocked(int mouse, int axis) {
        ...
        return covered ? DihOverlayManager.HOVER_BLOCKED_MOUSE : mouse;
    }
    ...
    protected int bodyMouseX(int mx, int my) {
        if (contextMenu != null && contextMenu.isMouseOver(mx, my)) {
            return DihOverlayManager.HOVER_BLOCKED_MOUSE;
        }
        return mx;
    }
