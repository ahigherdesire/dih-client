...
public final class DihPacketCapture {
    ...
    public static byte[] copyReadableBytes(ByteBuf source) {
        if (source == null) return DihNetworkCaptureState.EMPTY_BYTES;
        int length = Math.max(0, source.readableBytes());
        if (length == 0) return DihNetworkCaptureState.EMPTY_BYTES;
    ...
    public static String compactHex(byte[] bytes, int maxBytes) {
        return DihPayloadSupport.toCompactHex(bytes, maxBytes);
    }
