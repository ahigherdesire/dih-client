...
public final class DihPayloadEditorModel {
    ...
    public List<DihPayloadTemplate.Field> bodyFields = new ArrayList<>();
    ...
    public static DihPayloadEditorModel fromAction(PayloadAction action) {
        DihPayloadEditorModel model = new DihPayloadEditorModel();
        if (action == null) {
            model.bodyFields = new ArrayList<>(DihPayloadTemplate.defaultBrandTemplate().fields());
        ...
        if (action.payloadData != null && !action.payloadData.isBlank()) {
            try {
                raw = DihPayloadSupport.parsePayloadBytes(action.payloadData);
                haveRaw = true;
            } catch (Throwable ignored) {  }
        }
        List<DihPayloadTemplate.Field> fields = DihPayloadTemplate.parseFields(action.payloadFields);
        if (!fields.isEmpty()) {
    ...
    public PayloadAction toAction(PayloadAction base) {
        ...
        action.payloadFields = DihPayloadTemplate.serializeFields(bodyFields);
        action.payloadData = DihPayloadSupport.toHex(bodyBytes);
        action.payloadJson = "";
    ...
    public DihPayloadTemplate.Template toTemplate() {
        return new DihPayloadTemplate.Template(
            channel,
            DihPayloadTemplate.PayloadDirection.parse(direction),
            DihPayloadTemplate.PayloadPhase.parse(phase),
            DihPayloadTemplate.EncodingMode.parse(inferMode(), channel),
            bodyFields == null ? List.of() : bodyFields
        );
    }

    public boolean applyPacketScript(String script) {
        DihPayloadEditorModel parsed = copy();
        ...
        parsed.bodyFields = new ArrayList<>(DihPayloadTemplate.parseFields(String.join("\n", writerLines)));
    ...
    public boolean applyBodyHex(String hex) {
        try {
            ...
            validationError = DihPayloadSupport.safeMessage(t);
            touch();
            return false;
        }
    }
    ...
    public String packetScript() {
        ...
        sb.append('\n');
        sb.append(DihPayloadTemplate.serializeFields(bodyFields));
    ...
    public String utf8View() {
        ...
        String text = DihPayloadSupport.decodeLikelyUtf8Text(bodyBytes);
    ...
    public String logicalText() {
        ...
        sb.append("bodyHex = ").append(DihPayloadSupport.toHex(bodyBytes)).append('\n');
        if (packetId >= 0) {
            sb.append("logicalHex = ").append(DihPayloadSupport.toHex(logicalBytes())).append('\n');
        } else {
    ...
    public byte[] logicalBytes() {
        ...
        byte[] channelBytes = DihPayloadSupport.encodeMinecraftStringPayload(channel == null ? "" : channel);
    ...
    private boolean rebuildFromFields(boolean strict) {
        DihPayloadTemplate.Template template = toTemplate();
    ...
    private void setBodyBytes(byte[] bytes, Provenance source) {
        ...
        lastValidBytes = bodyBytes.clone();
        bodyFields = new ArrayList<>(DihPayloadSupport.inferEditablePayloadFields(bodyBytes));
        encodingMode = DihPayloadSupport.inferEditablePayloadMode(channel, bodyBytes);
        decodedKind = inferDecodedKind(bodyBytes);
    ...
    private void refreshCommandApiFromBytes() {
        Integer parsed = DihPayloadSupport.tryParseCommandApiValue(null, channel, bodyBytes);
        if (parsed != null) {
    ...
    private String inferMode() {
        ...
        return DihPayloadSupport.inferEditablePayloadMode(channel, bodyBytes);
    }

    private static String inferDecodedKind(byte[] bytes) {
        if (DihPayloadSupport.decodeMinecraftStringPayload(bytes) != null) return "Minecraft String";
        if (DihPayloadSupport.decodeJavaWriteUtfPayload(bytes) != null) return "Java writeUTF";
        String utf8 = DihPayloadSupport.decodeLikelyUtf8Text(bytes);
        if (utf8 != null && !utf8.isBlank()) return "Raw UTF-8";
        return "Binary Hex";
    }

    private DihPayloadEditorModel copy() {
        DihPayloadEditorModel copy = new DihPayloadEditorModel();
        copy.copyFrom(this);
        return copy;
    }

    private void copyFrom(DihPayloadEditorModel other) {
