...
public final class DihSvgHudLogo {
    private static final Minecraft MC = Minecraft.getInstance();
    private static final Identifier SVG_RESOURCE = Identifier.fromNamespaceAndPath("dihclient", "textures/gui/hud/dihclient.svg");
    private static final Identifier DYNAMIC_TEXTURE = Identifier.fromNamespaceAndPath("dihclient", "dynamic/hud/dihclient_svg");
    ...
    private static Entry entry(int width, int height) {
        ...
        try {
            ...
            MC.getTextureManager().register(id, new LinearDynamicTexture("DIH SVG HUD Logo " + key, image));
            releaseCurrent();
            ...
            if (++failCount >= MAX_FAILS) {
                disabled = true;
                DihClientAddon.LOG.warn("SVG HUD logo disabled after {} failed loads", failCount, t);
            }
            return null;
        }
    }
    ...
    private static NativeImage toNativeImage(BufferedImage image) {
        DihTheme.State theme = DihTheme.active();
        boolean recolor = theme.isActive(DihTheme.Channel.ACCENT);
        NativeImage nativeImage = new NativeImage(image.getWidth(), image.getHeight(), true);
        for (int y = 0; y < image.getHeight(); y++) {
            for (int x = 0; x < image.getWidth(); x++) {
                int argb = image.getRGB(x, y);
                if (recolor) argb = DihTheme.recolorImagePixel(argb, DihTheme.Channel.ACCENT, theme);
