...
import dihclient.gui.vanillaui.direct.DirectInfoRow;
...
import dihclient.gui.vanillaui.components.TabStrip;
...
public class DihServerInfoOverlay extends DihOverlayBase {
    private static final Minecraft MC = Minecraft.getInstance();
    private static final Logger LOGGER = LogUtils.getLogger();
    private static final String OVERLAY_ID = "dih-serverinfo";
    ...
    private static final String[] TAB_NAMES = dihclient.util.DihLiteVariant.enabled()
    ...
    private final DihPluginProbeIdAllocator pluginProbeIdAllocator = new DihPluginProbeIdAllocator();
    ...
    private static final boolean PLUGIN_SCAN_TRACE = Boolean.getBoolean("dih.plugin.scan.trace");
    ...
    private static final class PluginScanSession {
        ...
        final DihPluginScanJobState lifecycle;
        ...
        PluginScanSession(long generation, String serverAddress, String contextSignature, long startedAt,
            ...
            this.lifecycle = new DihPluginScanJobState(generation, this.serverAddress);
    ...
    private static final class RadarListRow {
        ...
        final String title;
        final DihDupeRadar.RadarMatch match;
        final DihDupeRadar.RadarFinding finding;
        final int height;
        final String searchText;

        private RadarListRow(RadarRowType type, String title, DihDupeRadar.RadarMatch match,
                             DihDupeRadar.RadarFinding finding, int height, String searchText) {
        ...
        static RadarListRow match(DihDupeRadar.RadarMatch match, int height, String searchText) {
            return new RadarListRow(RadarRowType.MATCH, "", match, null, height, searchText);
        }

        static RadarListRow finding(DihDupeRadar.RadarMatch match, DihDupeRadar.RadarFinding finding, int height, String searchText) {
    ...
    public DihServerInfoOverlay(Font textRenderer) {
    ...
    public void restoreState() {
        ...
        if (activeTab == 0) {
            applyInfoLayout();
        } else if (activeTab == 2 && !dihclient.util.DihLiteVariant.enabled()) {
            applyRadarLayout();
        } else {
    ...
    private void rebuildUi() {
        ...
        if (activeTab == 2 && !dihclient.util.DihLiteVariant.enabled()) {
            buildRadarUi();
            return;
        }
    ...
    private void buildInfoUi() {
        ...
        double estimatedTps = DihSharedState.get().getEstimatedTps();
        ...
        if (!"--".equals(versionNote)) addInfoRow("Version Note:", versionNote, DihColors.packetYellow());
        ...
        if (pluginScanInProgress) addInfoRow("AntiCheats:", "Scanning...", DihTheme.recolor(0xFFFF4444, DihTheme.Channel.DANGER));
        else if (!pluginScanDone) addInfoRow("AntiCheats:", "Probe Plugins First", DihTheme.recolor(0xFFFF4444, DihTheme.Channel.DANGER));
        else if (detectedAcs.isEmpty()) addInfoRow("AntiCheats:", "None detected", null);
        else addInfoRow("AntiCheats:", String.join(", ", detectedAcs), DihTheme.recolor(0xFFFF4444, DihTheme.Channel.DANGER));
    ...
    private void buildRadarUi() {
        DihDupeRadar.RadarState radarState = DihDupeRadar.state();
        ...
        if (!radarState.authenticated()) {
            ...
            if (radarState.error() != null && !radarState.error().isBlank()) {
                windowNode.content().add(new DirectInfoRow("Error:", radarState.error())
                    .setLabelWidth(38)
                    .setValueColorOverride(DihColors.dangerText()));
            }
            DirectRow loginButtons = new DirectRow().setGap(buttonRowGap());
            loginButtons.add(new DirectUiButton(radarState.authenticating() ? "Login..." : "Login DupeDB", DirectUiButton.Variant.SUCCESS, DihDupeRadar::login)
                .setGrowX(true)
        ...
        authButtons.add(new DirectUiButton("Logout", DirectUiButton.Variant.DANGER, DihDupeRadar::logout).setGrowX(true).setButtonHeight(actionButtonHeight()));
    ...
    private void syncRadarAuthState(DihDupeRadar.RadarState state) {
    ...
    private DirectUiButton autoProbeToggleButton() {
        boolean autoOn = DihConfig.getGlobal().autoProbePlugins;
    ...
    private void setPluginProbeDelayMs(int delayMs) {
        ...
        DihSharedState.get().setServerDataOverlayProbeDelayMs(pluginProbeDelayMs);
    }

    private void applyClampedBounds() {
        DihWindowLayout clamped = clampToViewport(new DihWindowLayout(panelX, panelY, panelW, panelH, visible, collapsed));
        panelX = clamped.x;
        panelY = clamped.y;
    ...
    private boolean isRadarAuthenticated() {
        DihDupeRadar.RadarState state = DihDupeRadar.state();
    ...
    private boolean loadCachedScan(String address, String contextSignature) {
        DihConfig.PluginScanCacheEntry cached = DihConfig.getGlobal().getPluginScan(address, contextSignature);

        if (cached == null) {
            cached = DihConfig.getGlobal().getPluginScanByAddress(address);
        }
    ...
    private void cacheCurrentScan() {
        ...
        DihConfig.getGlobal().putPluginScan(scannedServerAddress, "",
    ...
    private void syncScanStateForCurrentServer() {
        ...
        if (!currentAddress.equals(lastRadarServerAddress)) {
            ...
            if (!dihclient.util.DihLiteVariant.enabled()) DihDupeRadar.clearServerResults();
        }
    ...
    private String normalizePluginKey(String plugin) {
        return DihPluginPayloadFingerprints.canonicalPluginKey(plugin);
    }
    ...
    private int getConfidenceColor(PluginConfidence confidence, boolean hovered) {
        if (hovered) return DihColors.packetWhite();
        return switch (confidence == null ? PluginConfidence.UNKNOWN : confidence) {
            case EXACT -> DihColors.packetGreen();
            case STRONG -> DihColors.packetCyan();
            case FEATURE -> DihColors.packetYellow();
    ...
    private void tracePluginScan(PluginScanSession session, String message) {
        ...
        System.out.println("[DIH][PluginScan][gen=" + generation + "] " + message);
    }
    ...
    private boolean mergePayloadFingerprintEntries(Map<String, PluginScanEntry> entries) {
        ...
        boolean changed = false;
        for (DihPluginPayloadFingerprints.PluginFingerprint fingerprint : DihPluginPayloadFingerprints.pluginsFor(address, getLiveBrand())) {
    ...
    private PluginConfidence payloadFingerprintConfidence(DihPluginPayloadFingerprints.PluginFingerprint fingerprint) {
    ...
    private boolean promoteMatchingFeatureEntries(Map<String, PluginScanEntry> entries,
                                                  DihPluginPayloadFingerprints.PluginFingerprint fingerprint) {
    ...
    private Set<String> fingerprintKeywords(DihPluginPayloadFingerprints.PluginFingerprint fingerprint) {
    ...
    private void syncPayloadFingerprintsIntoCurrentScan() {
        ...
        String address = currentServerAddress();
        long revision = DihPluginPayloadFingerprints.revisionFor(address, getLiveBrand());
    ...
    public boolean autoProbeOnSpawn() {
        ...
        long cachedAt = DihConfig.getGlobal().getPluginScanTimestampByAddress(address);
        ...
        DihConfig.getGlobal().removePluginScan(address);
        preparePluginRescan();
        scanPlugins();
    ...
    private int getPluginScanPhaseColor() {
        PluginScanSession session = activePluginScan;
        return session == null || session.queuedProbes.isEmpty() ? DihColors.packetCyan() : DihColors.packetYellow();
    }
    ...
    private void announcePluginScanComplete() {
        ...
        if (count <= 0) {
            DihClientMessaging.sendPrefixed("Plugin probing finished: no plugins found.");
        } else if (count == 1) {
            DihClientMessaging.sendPrefixed("Plugin probing finished: found 1 plugin.");
        } else {
            DihClientMessaging.sendPrefixed("Plugin probing finished: found " + count + " plugins.");
        }
    }
    ...
    public void renderBackgroundProbeBanner(GuiGraphicsExtractor ctx) {
        ...
        try {
            ...
            dihclient.gui.vanillaui.components.ProgressBar.render(
                dihclient.gui.vanillaui.UiContexts.overlay(ctx, textRenderer, -1, -1),
    ...
    public void onCommandSuggestionRequest(net.minecraft.network.protocol.Packet<?> packet) {
        ...
        if (DihCommandSuggestionIds.isMacroId(id)) return;
    ...
    private void onCommandTreeChangedGuarded() {
        ...
        DihPluginPayloadFingerprints.clearSession();
        lastPayloadFingerprintRevision = -1L;
        if (currentAddress.equals(scannedServerAddress) && pluginScanDone
            && DihConfig.getGlobal().autoProbePlugins) {
            preparePluginRescan();
            scanPlugins(true);
    ...
    @Override public void setCollapsed(boolean c) {
        ...
        if (c) {
            ...
            else if (activeTab == 2 && !dihclient.util.DihLiteVariant.enabled()) applyRadarLayout();
    ...
    public void setBounds(DihWindowLayout b) {
        if (b == null) return;
        DihWindowLayout clamped = clampToViewport(b);
    ...
    private void renderGuarded(GuiGraphicsExtractor ctx, int mx, int my, float delta) {
        ...
        if (nowMs - lastUiRebuildMs >= 100L && (uiRebuildRequested || pluginScanInProgress)
            || activeTab == 2 && !dihclient.util.DihLiteVariant.enabled() && nowMs - lastUiRebuildMs >= 500L) {
            clickRegions.clear();
            rebuildUi();
        ...
        } else if (!collapsed && activeTab == 2 && !dihclient.util.DihLiteVariant.enabled() && isRadarAuthenticated()) {
            clickRegions.clear();
    ...
    private String getRealServerVersion() {
        String version = DihSharedState.get().getRealServerVersion(getDisplayedServerAddress());
            ...
            try {
                ...
                for (int rowIndex = startIndex; rowIndex < rows.size(); rowIndex++) {
                    ...
                    if (row.header()) {
                        ...
                        int nameColor = isAnticheat ? DihTheme.recolor(0xFFFF5555, DihTheme.Channel.DANGER) : getConfidenceColor(row.confidence, hovered);
                        ...
                        if (rowVisible) {
                            ...
                            if (!standaloneCommandHint) {
                                ...
                                clickRegions.add(new ClickRegion(clipLeft, hitTop, rowContentW, Math.max(1, hitBottom - hitTop), () -> {
                                    ...
                                    DihOverlayManager.get().setTemporarilyHidden(this, true);
    ...
    private int pluginDetailColor(PluginListRow row) {
        ...
        return switch (row.type) {
            ...
            case GUI -> DihColors.packetYellow();
    ...
    private void renderRadarRow(GuiGraphicsExtractor ctx, RadarListRow row, int x, int y, int width,
        ...
        if (row.type == RadarRowType.MATCH && row.match != null) {
            DihDupeRadar.RadarMatch match = row.match;
        ...
        if (row.type == RadarRowType.FINDING && row.finding != null && row.match != null) {
            DihDupeRadar.RadarFinding finding = row.finding;
            ...
            drawRadarChip(ctx, openX, y + 3, openW, row.height - 6, "Open", DihColors.packetCyan());
            int hitTop = Math.max(y, clipTop);
            int hitBottom = Math.min(y + row.height, clipBottom);
            clickRegions.add(new ClickRegion(openX, hitTop, openW, Math.max(1, hitBottom - hitTop), () -> DihDupeRadar.open(finding.sourceUrl())));
        }
    }

    private void drawRadarChip(GuiGraphicsExtractor ctx, int x, int y, int width, int height, String label, int color) {

        dihclient.gui.vanillaui.components.Chip.render(
            dihclient.gui.vanillaui.UiContexts.overlay(ctx, textRenderer, -1, -1),
    ...
    private int radarStatusColor(String status) {
        ...
        if (s.contains("unpatch") || s.contains("not patch")) return DihColors.packetGreen();
        if (s.contains("patch")) return RADAR_PATCHED_RED;
        if (s.contains("work")) return DihColors.packetGreen();
        if (s.contains("unverif") || s.contains("pending")) return DihColors.packetYellow();
        if (s.contains("verif")) return DihColors.packetGreen();
        return theme.color(UiTone.MUTED);
    }

    private int radarMatchStatusColor(DihDupeRadar.RadarMatch match) {
        if (match == null) return theme.color(UiTone.MUTED);
        boolean usable = false;
        boolean unverified = false;
        boolean patched = false;
        for (DihDupeRadar.RadarFinding finding : match.findings()) {
        ...
        if (usable) return DihColors.packetGreen();
        if (unverified) return DihColors.packetYellow();
    ...
    private int radarMatchStatusRank(DihDupeRadar.RadarMatch match) {
        int best = 3;
        if (match != null) {
            for (DihDupeRadar.RadarFinding finding : match.findings()) {
    ...
    private List<RadarListRow> buildRadarRows(int rowContentW) {
        ...
        List<DihDupeRadar.RadarMatch> matches = new ArrayList<>();
        for (DihDupeRadar.RadarMatch match : state.matches()) {
        ...
        matches.sort(Comparator.comparingInt((DihDupeRadar.RadarMatch m) -> radarMatchStatusRank(m))
            .thenComparing(Comparator.comparingLong(DihDupeRadar.RadarMatch::sortTimestampMs).reversed())
            .thenComparing(DihDupeRadar.RadarMatch::displayLabel, String.CASE_INSENSITIVE_ORDER));
        for (DihDupeRadar.RadarMatch match : matches) {
            rows.add(RadarListRow.match(match, RADAR_CARD_H, radarSearchBlob(match)));
            if (expandedRadarPlugins.contains(radarMatchKey(match))) {
                List<DihDupeRadar.RadarFinding> findings = new ArrayList<>(match.findings());
                findings.sort(Comparator.comparingInt((DihDupeRadar.RadarFinding f) -> radarStatusRank(f.status()))
                    .thenComparing(Comparator.comparingLong(DihDupeRadar.RadarFinding::sortTimestampMs).reversed()));
                for (DihDupeRadar.RadarFinding finding : findings) {
                    if (finding == null) continue;
    ...
    private boolean radarMatchPassesFilter(DihDupeRadar.RadarMatch match) {
        return match != null;
    }

    private String radarSearchBlob(DihDupeRadar.RadarMatch match) {
    ...
    private String radarFindingSearchBlob(DihDupeRadar.RadarMatch match, DihDupeRadar.RadarFinding finding) {
        if (finding == null) return "";
        return lowerSearch(match == null ? "" : match.detectedPlugin(), finding.title(), finding.summary(), finding.provider());
    }

    private String radarMatchKey(DihDupeRadar.RadarMatch match) {
        if (match == null) return "";
    ...
    private List<String> getPluginChannelDetailRows(String plugin) {
        ...
        List<String> probableNamespaces = DihPluginPayloadFingerprints.probableNamespacesForPlugin(plugin);
    ...
    private int channelDetailColor(String detail) {
        ...
        if (lower.contains("[registered")) return DihColors.packetGreen();
        if (lower.contains("[live")) return DihColors.packetCyan();
    ...
    private String bestGuessPayloadChannel(String rawChannel) {
        ...
        try {
            DihPayloadSupport.parseChannel(channel);
            return channel;
        } catch (IllegalArgumentException ignored) {
            return "";
        }
    ...
    private void copyClipboardValue(String value, String successMessage, String unavailableMessage) {
        ...
        if (trimmed.isEmpty() || "--".equals(trimmed) || "Resolving...".equalsIgnoreCase(trimmed) || "Failed".equalsIgnoreCase(trimmed)) {
            DihNotifications.error(unavailableMessage);
            return;
        }
        MC.keyboardHandler.setClipboard(trimmed);
        DihNotifications.copied(successMessage);
    }
    ...
    private boolean mouseClickedGuarded(double mx, double my, int button) {
        ...
        if (activeTab == 2 && !dihclient.util.DihLiteVariant.enabled() && isRadarAuthenticated()) {
        ...
        if ((activeTab == 1 && pluginScanDone && !pluginScanInProgress)
                || activeTab == 2 && !dihclient.util.DihLiteVariant.enabled() && isRadarAuthenticated()) {
    ...
    private List<DihDupeRadar.RadarPluginSnapshot> radarPluginSnapshots() {
        if (!pluginScanDone) return List.of();
        List<DihDupeRadar.RadarPluginSnapshot> snapshots = new ArrayList<>();
        for (PluginDetail detail : getCachedPluginDetails()) {
            if (detail == null || detail.displayName.isBlank()) continue;
            ...
            snapshots.add(new DihDupeRadar.RadarPluginSnapshot(
                detail.displayName,
                detail.key,
        ...
        snapshots.sort(Comparator.comparing(DihDupeRadar.RadarPluginSnapshot::displayName, String.CASE_INSENSITIVE_ORDER));
        return snapshots;
    }

    private void maybeAutoCheckRadar(DihDupeRadar.RadarState state, int pluginCount) {
        if (state == null || !state.authenticated() || state.authenticating() || state.checking()) return;
        DihDupeRadar.RadarServerSnapshot serverSnapshot = radarServerSnapshot();
        ...
        DihDupeRadar.checkServer(radarPluginSnapshots(), serverSnapshot, stale);
    }

    private String radarCheckSignature() {
        StringBuilder sb = new StringBuilder();
        sb.append(currentServerAddress()).append('|');
        for (DihDupeRadar.RadarPluginSnapshot snapshot : radarPluginSnapshots()) {
            sb.append(snapshot.canonicalKey()).append(',');
        }
        for (DihDupeRadar.RadarServerIdentity identity : radarServerSnapshot().identities()) {
            sb.append(identity.ip() ? "ip:" : "domain:").append(identity.value()).append(',');
        }
        return sb.toString();
    }

    private DihDupeRadar.RadarServerSnapshot radarServerSnapshot() {
        List<DihDupeRadar.RadarServerIdentity> identities = new ArrayList<>();
        ...
        return new DihDupeRadar.RadarServerSnapshot(identities);
    }
    ...
    private void addRadarServerIdentity(List<DihDupeRadar.RadarServerIdentity> identities, String raw) {
        ...
        identities.add(new DihDupeRadar.RadarServerIdentity(host, ip));
    }

    private void copyRadarReport() {
        MC.keyboardHandler.setClipboard(DihDupeRadar.copyReport());
        DihNotifications.copied("Radar report copied.");
    }
    ...
    private void copyServerData() {
        ...
        DihNotifications.copied("Server info copied.");
    }

    private void copyPluginList() {
        if (detectedPlugins.isEmpty()) {
            DihNotifications.error("No plugins detected.");
            return;
        }
        ...
        if (copyCount <= 0) {
            DihNotifications.error("No non-hint plugins to copy.");
            return;
        }
        StringBuilder sb = new StringBuilder();
        ...
        MC.keyboardHandler.setClipboard(sb.toString());
        DihNotifications.copied("Plugin list copied.");
    }
}
