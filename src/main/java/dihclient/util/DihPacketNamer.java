...
public class DihPacketNamer {
    ...
    private static String resolveRegistryName(Class<? extends Packet<?>> packetClass) {
        ...
        String direct = DihPacketRegistry.getName(packetClass);
        if (direct != null) return direct;

        for (Class<? extends Packet<?>> registeredClass : DihPacketRegistry.PACKETS) {
            if (registeredClass == packetClass || registeredClass.isAssignableFrom(packetClass)) {
                String name = DihPacketRegistry.getName(registeredClass);
    ...
    private static String resolveNameFromText(String text) {
        ...
        for (Class<? extends Packet<?>> registeredClass : DihPacketRegistry.PACKETS) {
            String registeredName = DihPacketRegistry.getName(registeredClass);
    ...
    private static String resolveDirectionalRegistryName(Class<? extends Packet<?>> packetClass, String direction) {
        for (Class<? extends Packet<?>> registeredClass : getDirectionalRegistry(direction)) {
            if (registeredClass == packetClass) {
                return DihPacketRegistry.getName(registeredClass);
            }
        }
        return null;
    }
    ...
    private static Iterable<Class<? extends Packet<?>>> getDirectionalRegistry(String direction) {
        if ("C2S".equalsIgnoreCase(direction)) return DihPacketRegistry.getC2SPackets();
        if ("S2C".equalsIgnoreCase(direction)) return DihPacketRegistry.getS2CPackets();
        return DihPacketRegistry.PACKETS;
    }
    ...
    public static String normalizePacketKey(String value) {
        ...
        return NORMALIZED_CACHE.computeIfAbsent(value, DihPacketNamer::normalizePacketKeyUncached);
    }
