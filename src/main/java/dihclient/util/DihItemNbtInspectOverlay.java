...
import dihclient.modules.DihAdminToolsBridge;
...
public class DihItemNbtInspectOverlay extends DihOverlayBase {
    private static final Minecraft MC = Minecraft.getInstance();
    private static final int SCROLLBAR_WIDTH = InspectorLayout.SCROLLBAR_WIDTH;
    private static DihItemNbtInspectOverlay sharedOverlay;

    private final Font textRenderer;
    private final CompactTheme theme = new CompactTheme();
    private DihItemNbtInspector.Inspection inspection;
    ...
    public DihItemNbtInspectOverlay(Font textRenderer) {
    ...
    public static DihItemNbtInspectOverlay getSharedOverlay(Font textRenderer) {
        if (textRenderer == null) return null;
        if (sharedOverlay == null) sharedOverlay = new DihItemNbtInspectOverlay(textRenderer);
    ...
    public static void dismissShared() {
        ...
        DihOverlayManager.get().unregister(sharedOverlay);
    }

    public static boolean openGlobal(ItemStack stack) {
        if (stack == null || stack.isEmpty() || MC.font == null) return false;
        DihItemNbtInspectOverlay overlay = getSharedOverlay(MC.font);
        if (overlay == null) return false;
        int screenW = MC.getWindow() == null ? 640 : DihUiScale.getVirtualScreenWidth();
        int x = Math.max(8, (screenW - 320) / 2);
        overlay.open(stack, x, 42);
        return true;
    }

    public static boolean openGlobal(DihItemNbtInspector.Inspection inspection) {
    ...
    public void open(ItemStack stack, int anchorX, int anchorY) {
        ...
        open(DihItemNbtInspector.inspect(stack), anchorX, anchorY);
    }

    public void open(DihItemNbtInspector.Inspection inspection, int anchorX, int anchorY) {
        ...
        this.wrapDirty = true;
        int screenW = MC.getWindow() == null ? 640 : DihUiScale.getVirtualScreenWidth();
        int screenH = MC.getWindow() == null ? 360 : DihUiScale.getVirtualScreenHeight();
        ...
        setBounds(new DihWindowLayout(Math.max(8, Math.min(anchorX, screenW - width - 8)),
    ...
    private List<DihItemNbtInspector.InspectionLine> currentLines() {
    ...
    private void ensureWrappedLines(int maxWidth) {
        ...
        for (DihItemNbtInspector.InspectionLine line : currentLines()) {
    ...
    private void wrapLine(DihItemNbtInspector.InspectionLine line, int maxWidth, List<WrappedInspectionLine> target) {
    ...
    private void wrapKeyValue(String prefix, String value, int valueColor, int prefixWidth, int continuationOffset, int maxWidth,
        ...
        if (value == null || value.isEmpty()) {
            target.add(WrappedInspectionLine.of(List.of(
                new DihItemNbtInspector.TextToken(prefix, DihColors.packetGray()),
                new DihItemNbtInspector.TextToken("", valueColor)
            ), 0));
            return;
        }
    ...
    private void drawInspectionLine(GuiGraphicsExtractor context, WrappedInspectionLine line, int x, int y) {
        ...
        for (DihItemNbtInspector.TextToken token : line.tokens()) {
    ...
    private void wrapTokenLine(List<DihItemNbtInspector.TextToken> source, int maxWidth, List<WrappedInspectionLine> target) {
        if (source == null || source.isEmpty()) {
            target.add(WrappedInspectionLine.plain("", 0, DihColors.textMuted()));
            return;
        }
        ...
        List<DihItemNbtInspector.TextToken> current = new ArrayList<>();
        int currentWidth = 0;
        boolean continuation = false;

        for (DihItemNbtInspector.TextToken token : source) {
            if (token == null || token.text() == null || token.text().isEmpty()) continue;
            ...
            while (!remaining.isEmpty()) {
                ...
                current.add(new DihItemNbtInspector.TextToken(piece, token.color()));
    ...
    private WrappedInspectionLine keyValueWrappedLine(String prefix, String value, int valueColor, int offset) {
        List<DihItemNbtInspector.TextToken> tokens = new ArrayList<>();
        if (prefix != null && !prefix.isEmpty()) tokens.add(new DihItemNbtInspector.TextToken(prefix, DihColors.packetGray()));
        tokens.add(new DihItemNbtInspector.TextToken(value == null ? "" : value, valueColor));
        return WrappedInspectionLine.of(tokens, prefix == null || prefix.isEmpty() ? offset : 0);
    }

    private int leadingWhitespaceWidth(List<DihItemNbtInspector.TextToken> tokens) {
        int width = 0;
        for (DihItemNbtInspector.TextToken token : tokens) {
            if (token == null || token.text() == null || token.text().isEmpty()) continue;
    ...
    private void handleFooter(String label) {
        ...
        switch (label) {
            case "Copy Raw" -> {
                ...
                DihNotifications.copied("Copied raw " + inspection.subject() + " NBT.");
            }
            case "Copy Nice" -> {
                MC.keyboardHandler.setClipboard(inspection.prettyCopyText());
                DihNotifications.copied("Copied " + inspection.subject() + " NBT summary.");
            }
            case "Fill Editor" -> {

                if (!dihclient.util.DihLiteVariant.enabled()
                    && inspection.stack() != null && !inspection.stack().isEmpty()
                    && DihAdminToolsBridge.openFilledAdminEditor(inspection.stack())) {
                    DihNotifications.show("Filled Admin Tools with complete item NBT.", 0xFF35D873);
                } else {
                    DihClientMessaging.sendPrefixed("Admin Tools: could not fill NBT editor.");
                }
            }
        }
    }
    ...
    private List<FooterButton> footerButtons(int y) {
        ...
        boolean editor = !dihclient.util.DihLiteVariant.enabled()
    ...
    private record WrappedInspectionLine(List<DihItemNbtInspector.TextToken> tokens, int offset) {
        static WrappedInspectionLine plain(String text, int offset, int color) {
            return new WrappedInspectionLine(List.of(new DihItemNbtInspector.TextToken(text, color)), offset);
        }

        static WrappedInspectionLine of(List<DihItemNbtInspector.TextToken> tokens, int offset) {
