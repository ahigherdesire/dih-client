...
public final class DihInstaBreakRenderer {
    ...
    public static void initialize() {
        LevelRenderEvents.COLLECT_SUBMITS.register(context -> {
            ...
            if (drawSides(finalShape) && ((finalSideColor >>> 24) & 0xFF) > 0) {
                context.submitNodeCollector().submitCustomGeometry(poseStack, DihRenderTypes.storageEspFillSeeThrough(), (pose, buffer) -> fillBox(pose, buffer, box, finalSideColor));
            }
            if (drawLines(finalShape)) {
                context.submitNodeCollector().submitCustomGeometry(poseStack, DihRenderTypes.storageEspLinesSeeThrough(), (pose, buffer) -> renderBox(pose, buffer, box, finalColor));
            }
        });
    }
    ...
    private static void line(PoseStack.Pose pose, VertexConsumer buffer, double x1, double y1, double z1, double x2, double y2, double z2, int color) {
        DihWorldGeometry.line(pose, buffer, x1, y1, z1, x2, y2, z2, color, LINE_WIDTH);
    }
