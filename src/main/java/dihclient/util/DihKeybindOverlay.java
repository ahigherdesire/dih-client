...
import dihclient.gui.vanillaui.direct.DirectFormRow;
...
public class DihKeybindOverlay extends DihOverlayBase {
    ...
    public DihKeybindOverlay() {
        super("dih-keybinds", MIN_PANEL_WIDTH, 142);
        this.panelX = 160;
        this.panelY = 46;

        if (!dihclient.util.DihLiteVariant.enabled()) {
            entries.add(new KeybindEntry("Module Menu", "Opens the Dih module click GUI", () -> getConfig().keybindModuleMenu, v -> getConfig().keybindModuleMenu = v));
        }
    ...
    private void openThemeColor() {
        MC.gui.setScreen(new dihclient.gui.screen.DihThemeColorScreen(MC.gui.screen()));
    }
    ...
    private void openProfiles() {
        ...
        dihclient.util.IDihOverlay profiles = module == null ? null : module.getProfilesOverlay();
        if (profiles == null) return;
        ((dihclient.util.DihProfilesOverlay) profiles).setMainMenuMode(false);
    ...
    private DihConfig getConfig() {
        return DihConfig.getGlobal();
    }
    ...
    public static String getKeyName(int keyCode) {
        return DihBindUtil.getBindName(keyCode);
    }
    ...
    public boolean mouseClicked(double mouseX, double mouseY, int button) {
        ...
        if (capturingIndex >= 0 && DihBindUtil.isAllowedMouseButton(button)) {
            captureBind(DihBindUtil.encodeMouseButton(button));
            return true;
        }
    ...
    private final class ToggleRowNode extends DirectFormRow {
        ...
        public void render(DirectRenderContext context) {
            ...
            dihclient.gui.vanillaui.UiBounds pillBounds = dihclient.gui.vanillaui.UiBounds.of(
                Math.round(toggleButton.x()), Math.round(toggleButton.y()),
                Math.round(toggleButton.width()), Math.round(toggleButton.height()));
            dihclient.gui.vanillaui.components.Toggle.render(ui, pillBounds, on,
    ...
    private final class PrefixRowNode extends DirectUiNode {
        ...
        public void render(DirectRenderContext context) {
            ...
            String value = DihCompatManager.effectiveCommandPrefix();
