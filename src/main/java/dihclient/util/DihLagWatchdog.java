...
public final class DihLagWatchdog {
    private static final int SAMPLE_EVERY_TICKS = 20;
    private static final int FORCED_WINDOW_TICKS = 200;
    private static final boolean TEST_MODE = Boolean.getBoolean("dih.lagtest");
    ...
    private DihLagWatchdog() {
    }

    public static void onClientTick(Minecraft client) {
        try {
            DihPerf.tickForcedWindow();
            if (censusPending && !DihPerf.isForcedWindowActive()) {
                censusPending = false;
                emitCensus(client);
            }
            ...
            if (now - lastErrorLogMs >= 5000L) {
                lastErrorLogMs = now;
                DihClientAddon.LOG.warn("[Dih] lag watchdog failed; isolated to protect the tick", t);
            }
        }
    }
    ...
    private static void sample(Minecraft client) {
        ...
        if (!DihPerf.enabled()) {
            ...
            DihClientAddon.LOG.warn(
                "[DihLagReport] sustained low fps detected (fps={} baseline={})",
                fps, String.format(java.util.Locale.ROOT, "%.1f", baselineFps));
            return;
        }
        countersAtTrigger = DihPerf.snapshotCounters();
        DihPerf.beginForcedWindow(FORCED_WINDOW_TICKS);
        censusPending = true;
        DihClientAddon.LOG.warn(
            "[DihLagReport] sustained low fps detected (fps={} baseline={}); sampling frame time for 10s...",
            fps, String.format(java.util.Locale.ROOT, "%.1f", baselineFps));
    }
    ...
    private static void emitCensus(Minecraft client) {
        ...
        appendSafe(sb, "state", () -> {
            ...
            return "focused=" + focused
                ...
                + " capture=" + capture
                + " payloadStudy=" + DihPayloadStudySession.isActive()
                + " packHide=" + PackHideState.isActive();
        });
        appendSafe(sb, "overlays", () -> DihOverlayManager.get().censusSummary());
        appendSafe(sb, "packetLogger", () -> {
            DihModule module = DihModule.get();
            DihPacketLoggerOverlay logger = module == null ? null : module.getPacketLoggerOverlayIfExists();
        ...
        appendSafe(sb, "hud", () -> {
            ...
            if (!dihclient.util.DihLiteVariant.enabled()) {
                mmSegment = " mm=" + dihclient.util.mm.MatchmakingManager.get().censusSummary();
            }
            return "enabledElements=" + DihHudManager.enabledElementCount()
                + " activeModules=" + ModuleRegistry.activeModules().size()
                + " toasts=" + DihNotifications.pendingCount()
                + mmSegment;
        });
        appendSafe(sb, "frame sections (10s window, window-total desc, avg/max* ms, n)", () -> {
            ...
            for (Map.Entry<String, long[]> entry : DihPerf.snapshotCounters().entrySet()) {
        ...
        countersAtTrigger = null;
        DihClientAddon.LOG.warn("[DihLagReport] {}", sb);
    }

    private static void line(StringBuilder sb, String text) {
        sb.append("\n[DihLagReport] ").append(text);
    }
