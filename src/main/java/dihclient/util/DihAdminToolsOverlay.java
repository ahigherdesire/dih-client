...
import dihclient.gui.screen.DihItemPickerScreen;
...
import dihclient.gui.vanillaui.components.CompactFieldFactory;
import dihclient.gui.vanillaui.components.CompactForm;
import dihclient.gui.vanillaui.components.CompactToolbar;
...
public final class DihAdminToolsOverlay extends DihOverlayBase {
    private static volatile DihAdminToolsOverlay sharedOverlay;
    ...
    private DihAdminToolsOverlay() {
        super("admin-tools-panel", DEFAULT_W, DEFAULT_H);
        panelX = 88;
        panelY = 32;
    }

    public static synchronized DihAdminToolsOverlay getSharedOverlay() {
        if (sharedOverlay == null) sharedOverlay = new DihAdminToolsOverlay();
        return sharedOverlay;
    }

    public static DihAdminToolsOverlay getSharedOverlayIfExists() {
        return sharedOverlay;
    }
    ...
    private void renderBody(UiContext context, BuiltinModules.AdminToolsModule module, DihWindowLayout bounds, int mouseX, int mouseY) {
        int x = bounds.x + 5;
    ...
    private void renderItemRaw(UiContext context, BuiltinModules.AdminToolsModule module, Flow flow, int mouseX, int mouseY) {
        ...
        List<String> lines = DihItemNbtInspector.prettySnbtLines(module.value("nbt-item-stack"));
        int x = preview.x() + 7;
        int y = preview.y() + 18;
    ...
    private void drawRawPreviewLine(UiContext context, String line, int x, int y, int maxWidth) {
        ...
        for (DihItemNbtInspector.TextToken token
            : DihItemNbtInspector.tokenizeStructuredText(line, context.theme().colors().text)) {
    ...
    private void renderBindRow(UiContext context, Module module, int x, int y, int w, String label, String optionId, int mouseX, int mouseY) {
        ...
        Button.render(context, bind, focused ? "Press key" : DihBindUtil.getBindName(parseInt(module.value(option.id()), -1)), Button.Tone.NORMAL, bind.contains(mouseX, mouseY), focused);
    ...
    private void handleAction(BuiltinModules.AdminToolsModule module, Hit hit) {
        ...
        switch (hit.value) {
            ...
            case "edit-lore" -> openItemOptionEditor(module, "nbt-item-lore", DihAdminItemOptionScreen.Mode.LORE);
            case "edit-enchants" -> openItemStructuredEditor(module, "nbt-enchants", DihAdminItemStructuredScreen.Mode.ENCHANTMENTS);
            case "edit-attributes" -> openItemStructuredEditor(module, "nbt-attributes", DihAdminItemStructuredScreen.Mode.ATTRIBUTES);
            case "edit-custom-data" -> openItemOptionEditor(module, "nbt-custom-data", DihAdminItemOptionScreen.Mode.RAW);
            case "edit-item-stack" -> {
                module.prepareRawItemStackEditor();
                openItemOptionEditor(module, "nbt-item-stack", DihAdminItemOptionScreen.Mode.FULL_NBT);
            }
            case "copy-item-stack-snbt" -> {
                ...
                if (mc == null || mc.keyboardHandler == null || raw == null || raw.isBlank()) {
                    DihNotifications.error("No full ItemStack NBT to copy.");
                } else {
                    mc.keyboardHandler.setClipboard(raw);
                    DihNotifications.copied("Copied full ItemStack NBT.");
                }
            }
            ...
            case "forceop-preview" -> {
                Minecraft mc = Minecraft.getInstance();
                if (mc != null) mc.gui.setScreen(new DihForceOpPreviewScreen(mc.gui.screen(), module.getForceOpPasswords()));
            }
            default -> {
            }
        }
    ...
    private void openItemOptionEditor(Module module, String optionId, DihAdminItemOptionScreen.Mode mode) {
        Minecraft mc = Minecraft.getInstance();
        Setting<?, ?> option = option(module, optionId);
        if (mc != null && option != null) mc.gui.setScreen(new DihAdminItemOptionScreen(mc.gui.screen(), module, option, mode));
    }

    private void openItemStructuredEditor(Module module, String optionId, DihAdminItemStructuredScreen.Mode mode) {
        Minecraft mc = Minecraft.getInstance();
        Setting<?, ?> option = option(module, optionId);
        if (mc != null && option != null) mc.gui.setScreen(new DihAdminItemStructuredScreen(mc.gui.screen(), module, option, mode));
    }
    ...
    private void openItemPicker(Module module, UiBounds anchor) {
        ...
        Minecraft mc = Minecraft.getInstance();
        if (mc != null) {
            mc.gui.setScreen(new DihItemPickerScreen(mc.gui.screen(), module.value("nbt-item-id"), value -> {
    ...
    private void startInventoryItemPicker(BuiltinModules.AdminToolsModule module) {
        ...
        if (mc == null || mc.player == null) {
            DihNotifications.warning("Join a world first.");
            return;
        }
        finishEditing(true);
        clearDropdown();
        ...
        DihSharedState.get().setCaptureCancelCallback(this::cancelInventoryItemPicker);
        DihNotifications.show("Right-click an inventory item to pick it.", 0xFF35D873);
    ...
    public String getAbstractContainerScreenCaptureHoverText(Slot slot, String itemName, String registryId) {
        ...
        int visibleSlot = DihInventoryHelper.toUserVisibleSlot(Minecraft.getInstance(), slot);
    ...
    public boolean onInventoryItemStackCapture(Slot slot) {
        ...
        if (module == null || stack.isEmpty()) {
            DihNotifications.warning("Pick a non-empty item slot.");
            return true;
        }
        module.fillItemEditorFromStack(stack, false);
        DihNotifications.show("NBT editor filled from " + BuiltInRegistries.ITEM.getKey(stack.getItem()) + ".", 0xFF35D873);
    ...
    public void cancelInventoryItemPicker() {
        ...
        DihNotifications.warning("Item pick cancelled.");
        finishInventoryItemPicker();
    }
    ...
    private void finishInventoryItemPicker() {
        ...
        DihSharedState.get().setCaptureCancelCallback(null);
