...
public final class DihPacketSchemaRegistry {
    private static volatile Map<String, PacketSchema> schemas;

    private DihPacketSchemaRegistry() {
    }
    ...
    private static Map<String, PacketSchema> load() {
        ...
        synchronized (DihPacketSchemaRegistry.class) {
            current = schemas;
            if (current != null) return current;
            Map<String, PacketSchema> loaded = new LinkedHashMap<>();
            try (InputStream in = DihPacketSchemaRegistry.class.getResourceAsStream("/dih-packet-schemas.tsv")) {
                if (in != null) {
