...
import dihclient.util.mm.ServerClock;
...
public final class DihDiscordLogin {

    public static final String CLIENT_ID = "1520738158457655407";
    public static final String AUTH_BASE = "https://auth.dihclient.com";
    ...
    public static String errorMessage(String code) {
        return switch (code == null ? "" : code) {
            ...
            case "old_version" -> "Update needed. dihclient.com";
    ...
    public static synchronized void signOut() {
        ...
        dihclient.util.mm.MatchmakingManager.get().resetAutoJoinAttempt();

        try {
            dihclient.util.mm.MatchmakingManager.get().closeRelays();
    ...
    public static String freshJwt(long minTtlMs) {
        ...
        synchronized (DihDiscordLogin.class) {
        ...
        try {
            String form = "session=" + enc(session) + "&version=" + enc(VERSION) + "&pubkey=" + enc(selfPubKeyB64())
                + DihAttest.formSuffix(AUTH_BASE, VERSION);
            ...
            DihHttp.JsonResult result = DihHttp.postFormResult(AUTH_BASE + "/token", form, proof);
            ...
            if ((status == 401 || (r != null && r.has("error") && "proof_required".equals(r.get("error").getAsString())))
                ...
                try {
                    String retryForm = "session=" + enc(session) + "&version=" + enc(VERSION)
                        + "&pubkey=" + enc(selfPubKeyB64()) + DihAttest.formSuffix(AUTH_BASE, VERSION);
                    Map<String, String> retryProof = proofHeaders("POST", "/token", retryForm);
                    if (!retryProof.isEmpty()) {
                        DihHttp.JsonResult retry = DihHttp.postFormResult(AUTH_BASE + "/token",
        ...
        synchronized (DihDiscordLogin.class) {
            ...
            if (status == 200 && r != null && r.has("error")) {
                ...
                if (isDefinitiveRevocation(code) && confirmDefinitive(code, now)) {
                    DihNotifications.show(errorMessage(code), 0xFFFF5B5B);
                    signOut();
                }
            }
            return jwt.isEmpty() || now >= jwtExpMs ? null : jwt;
    ...
    private static void startServer() {
        ...
        try {
            ...
            s.createContext("/", DihDiscordLogin::handle);
    ...
    private static String exchangeCode(String code) {
        try {
            JsonObject r = DihHttp.postForm(AUTH_BASE + "/login",
