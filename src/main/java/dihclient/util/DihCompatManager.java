...
public final class DihCompatManager {
    ...
    private static volatile int cachedDihModuleRevision = Integer.MIN_VALUE;
    private static volatile List<String> cachedDihModuleNames = List.of();
    private static volatile List<String> cachedMeteorOnlyModuleNames;

    private DihCompatManager() {
    }
    ...
    public static String effectiveCommandPrefix() {
        ...
        String stored = module == null ? DihConfig.getGlobal().commandPrefix : module.getCommandPrefix();
    ...
    public static String meteorCommandPrefix() {
        ...
        synchronized (DihCompatManager.class) {
            now = System.nanoTime();
            if (now < meteorPrefixRefreshAtNanos) return cachedMeteorCommandPrefix;
    ...
    public static List<DihMacro> getMeteorMacros() {
        ...
        try {
            ...
            for (Object meteorMacro : iterable) {
                DihMacro converted = convertMeteorMacro(meteorMacro);
                if (converted != null) macros.add(converted);
            }
        } catch (Exception e) {
            DihClientAddon.LOG.warn("[Dih] Failed to read Meteor macros reflectively.", e);
        }
        return macros;
    }
    ...
    public static List<String> getMeteorMacroNames() {
        ...
        for (DihMacro macro : getMeteorMacros()) {
            if (macro != null && macro.name != null && !macro.name.isBlank()) names.add(macro.name);
        }
        return names;
    }

    public static DihMacro getMeteorMacro(String macroName) {
    ...
    public static boolean importToMeteor(DihMacro packUtilMacro) {
        return false;
    }

    public static List<String> getDihModuleNames() {
        int revision = ModuleRegistry.revision();
        List<String> names = cachedDihModuleNames;
        if (cachedDihModuleRevision != revision) {
            ...
            cachedDihModuleNames = names;
            cachedDihModuleRevision = revision;
        }
    ...
    public static List<String> getMeteorOnlyModuleNames() {
        ...
        List<String> nativeNames = getDihModuleNames();
        List<String> names = new ArrayList<>();
        try {
            ...
            DihClientAddon.LOG.warn("[Dih] Failed to read Meteor modules reflectively.", e);
        }
        return names;
    }
    ...
    public static List<String> getMeteorModuleNames() {
        List<String> names = getDihModuleNames();
    ...
    public static boolean toggleMeteorModule(String moduleName, ToggleModuleAction.ToggleMode toggleMode) {
        ...
        try {
            ...
            DihClientAddon.LOG.warn("[Dih] Failed to toggle Meteor module '{}' reflectively.", moduleName, e);
            return false;
        }
    }
    ...
    public static boolean startBaritoneGoTo(Minecraft mc, int x, int y, int z) {
        ...
        try {
            ...
            DihClientAddon.LOG.warn("[Dih] Failed to start Baritone goto reflectively, falling back to chat command.", t);
    ...
    public static boolean startBaritoneMine(Minecraft mc, List<String> blockNames) {
        ...
        try {
            ...
            DihClientAddon.LOG.warn("[Dih] Failed to start Baritone mine reflectively, falling back to chat command.", t);
    ...
    public static void stopBaritone(Minecraft mc) {
        ...
        try {
            ...
            DihClientAddon.LOG.warn("[Dih] Failed to stop Baritone reflectively, falling back to chat command.", t);
        }
    ...
    private static DihMacro convertMeteorMacro(Object meteorMacro) {
        ...
        try {
            ...
            if (name == null || messages == null) {
                try {
                    ...
                    DihClientAddon.LOG.warn("[Dih] Settings iteration fallback failed.", e);
                }
            }
            ...
            DihClientAddon.LOG.warn("[Dih] Error converting Meteor macro.", e);
            return null;
        }
    }
