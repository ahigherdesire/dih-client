...
public class DihAccount {
    ...
    public DihAccountType type = DihAccountType.Cracked;
    public String label = "";
    public String token = "";
    ...
    public boolean login() {
        ...
        boolean needFetch = username == null || username.isBlank()
            || (type == DihAccountType.Microsoft && !hasFreshSessionToken());
        if (needFetch && !fetchInfo()) return false;
        return switch (type) {
            case Cracked, Generated -> {
                boolean ok = DihAccountSessionSwitcher.setSession(new User(username, UUIDUtil.createOfflinePlayerUUID(username), "", Optional.empty(), Optional.empty()));
            ...
            case Session, Microsoft -> {
                ...
                boolean ok = DihAccountSessionSwitcher.setSession(new User(username, UndashedUuid.fromStringLenient(uuid), token, Optional.empty(), Optional.empty()));
                if (!ok) lastError = DihAccountSessionSwitcher.lastError();
                yield ok;
            }
    ...
    private boolean fetchSession() {
        ...
        try {
            var profile = DihHttp.getJsonDirect("https://api.minecraftservices.com/minecraft/profile", token);
    ...
    private boolean fetchMicrosoft() {
        ...
        DihMicrosoftLogin.LoginData data = DihMicrosoftLogin.login(label);
        if (!data.isGood()) {
    ...
    private boolean fetchTheAltening() {
        ...
        try {
            ...
            lastError = shortError(e);
            if (!suppressMessages) DihClientMessaging.sendPrefixed("Invalid TheAltening credentials.");
            return false;
        } catch (Exception e) {
            clearTheAlteningAuth();
            lastError = shortError(e);
            if (!suppressMessages) DihClientMessaging.sendPrefixed("Failed to fetch TheAltening account info.");
            return false;
        }
    }
    ...
    private boolean loginTheAltening() {
        ...
        try {
            ...
            clearTheAlteningAuth();
            lastError = shortError(e);
            DihClientMessaging.sendPrefixed("Failed to login with TheAltening.");
            return false;
        }
    }
    ...
    public CompoundTag toTag() {
        ...
        tag.putString("type", type == null ? DihAccountType.Cracked.name() : type.name());
    ...
    public DihAccount fromTag(CompoundTag tag) {
        ...
        String typeName = tag.getStringOr("type", DihAccountType.Cracked.name());
        try {
            type = DihAccountType.valueOf(typeName);
    ...
    public boolean equals(Object obj) {
        ...
        if (!(obj instanceof DihAccount account)) return false;
