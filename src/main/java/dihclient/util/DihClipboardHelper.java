...
public class DihClipboardHelper {
    private static final int CLIPBOARD_VERSION = 1;
    private static final String MACRO_CLIPBOARD_TYPE = "dih_macro";
    private static final String MACRO_STEPS_CLIPBOARD_TYPE = "dih_macro_steps";

    public static void copyToClipboard(List<DihSharedState.QueuedPacket> queue) {
        try {
            CompoundTag rootTag = new CompoundTag();
            ListTag packetList = new ListTag();

            for (DihSharedState.QueuedPacket qp : queue) {
            ...
            DihClientAddon.LOG.info("[Dih] Copied {} packets to clipboard", queue.size());

        } catch (Exception e) {
            DihClientAddon.LOG.error("[Dih] Failed to copy packets to clipboard", e);
        }
    }

    public static boolean copyMacroToClipboard(DihMacro macro) {
        ...
        try {
            ...
            Minecraft.getInstance().keyboardHandler.setClipboard(base64);
            DihClientAddon.LOG.info("[Dih] Copied macro '{}' to clipboard", macro.name);
            return true;
        } catch (Exception e) {
            DihClientAddon.LOG.error("[Dih] Failed to copy macro to clipboard", e);
            return false;
        }
    }
    ...
    private static CompoundTag readClipboardRoot() {
        try {
            ...
            if (version != CLIPBOARD_VERSION) {
                DihClientAddon.LOG.warn("[Dih] Unsupported clipboard version: {}", version);
                return null;
            }
            return rootTag;
        } catch (Exception e) {
    ...
    private static List<MacroAction> deserializeActionList(ListTag actionList) {
        ...
        for (int i = 0; i < actionList.size(); i++) {
            ...
            if (!DihMacro.isBuiltInActionType(actionTag.getStringOr("type", ""))) { dropped++; continue; }
            MacroAction action = DihMacro.createActionFromTag(actionTag);
            if (action != null) {
    ...
    private static DihMacro buildUntrustedMacro(CompoundTag macroTag) {
        int dropped = DihMacro.stripToBuiltInActions(macroTag);
    ...
    public static DihMacro pasteMacroFromClipboard() {
        ...
        if (MACRO_CLIPBOARD_TYPE.equals(type)) {
            ...
            if (macroTag.isEmpty()) {
                DihClientAddon.LOG.warn("[Dih] Macro clipboard payload was empty");
                return null;
            }
        ...
        DihClientAddon.LOG.warn("[Dih] Clipboard data is not a Dih macro payload");
        return null;
    }
    ...
    public static String serializeMacroToBase64(DihMacro macro) {
        ...
        try {
            ...
            DihClientAddon.LOG.error("[Dih] Failed to serialize macro to Base64", e);
            return null;
        }
    }

    public static DihMacro deserializeMacroFromBase64(String base64) {
    ...
    public static boolean copyMacroStepsToClipboard(List<MacroAction> actions) {
        ...
        try {
            ...
            Minecraft.getInstance().keyboardHandler.setClipboard(base64);
            DihClientAddon.LOG.info("[Dih] Copied {} macro steps to clipboard", actionList.size());
            return true;
        } catch (Exception e) {
            DihClientAddon.LOG.error("[Dih] Failed to copy macro steps to clipboard", e);
            return false;
        }
    }
    ...
    public static List<DihSharedState.QueuedPacket> pasteFromClipboard() {
        try {
            String base64 = Minecraft.getInstance().keyboardHandler.getClipboard();
            if (base64 == null || base64.trim().isEmpty()) {
                DihClientAddon.LOG.warn("[Dih] Clipboard is empty");
                return null;
            }
            ...
            int version = rootTag.getIntOr("version", 0);
            if (version != 1) {
                DihClientAddon.LOG.error("[Dih] Unsupported clipboard format version: {}", version);
                return null;
            }

            ListTag packetList = (ListTag) rootTag.get("packets");
            List<DihSharedState.QueuedPacket> queue = new ArrayList<>();

            for (int i = 0; i < packetList.size(); i++) {
                CompoundTag packetTag = (CompoundTag) packetList.get(i);
                DihSharedState.QueuedPacket qp = deserializeQueuedPacket(packetTag);
                if (qp != null) {
                    queue.add(qp);
                }
            }

            DihClientAddon.LOG.info("[Dih] Pasted {} packets from clipboard", queue.size());
            return queue.isEmpty() ? null : queue;

        } catch (Exception e) {
            DihClientAddon.LOG.error("[Dih] Failed to paste packets from clipboard", e);
            return null;
        }
    }
    ...
    public static Packet<?> deserializePacketFromBase64(String base64) {
        try {
            ...
            if (tag.contains("packets")) {
                ...
                DihSharedState.QueuedPacket qp = deserializeQueuedPacket(tag);
                return qp != null ? qp.packet : null;
            } else {
            ...
            DihClientAddon.LOG.error("[Dih] Failed to deserialize packet from Base64", e);
            return null;
        }
    }

    public static CompoundTag serializeQueuedPacket(DihSharedState.QueuedPacket qp) {
        try {
            ...
            DihClientAddon.LOG.error("[Dih] Failed to serialize queued packet", e);
            return null;
        }
    }

    public static DihSharedState.QueuedPacket deserializeQueuedPacket(CompoundTag tag) {
        try {
            int delay = tag.getIntOr("delay", 0);
            int id = tag.getIntOr("id", 0);
            DihSharedState.ReplayMode replayMode = DihSharedState.ReplayMode.REGENERATE;
            if (tag.contains("replayMode")) {
                try {
                    replayMode = DihSharedState.ReplayMode.valueOf(tag.getStringOr("replayMode", "REGENERATE"));
                } catch (IllegalArgumentException ignored) {
                    replayMode = DihSharedState.ReplayMode.REGENERATE;
                }
            }
            ...
            if (packet != null) {
                return id > 0
                    ? new DihSharedState.QueuedPacket(packet, delay, id, replayMode)
                    : new DihSharedState.QueuedPacket(packet, delay, replayMode);
            }

            return null;
        } catch (Exception e) {
            DihClientAddon.LOG.error("[Dih] Failed to deserialize queued packet", e);
            return null;
        }
    }
    ...
    private static CompoundTag serializePacket(Packet<?> packet) {
        ...
        String friendlyName = DihPacketRegistry.getName((Class<? extends Packet<?>>) packet.getClass());
        ...
        if (packet instanceof ServerboundCustomPayloadPacket) {
            ...
            DihPayloadSupport.PayloadSnapshot snapshot = DihPayloadSupport.snapshot(packet, "C2S");
            if (snapshot != null) {
    ...
    private static Packet<?> deserializePacket(CompoundTag tag) {
        try {
            if (tag.getBooleanOr("customPayloadPacket", false)) {
                ...
                if (!channel.isBlank()) {
                    return DihPayloadSupport.createC2SPacket(channel, bytes);
                }
            }
            ...
            if (packetClass == null && tag.contains("friendlyName")) {
                packetClass = DihPacketRegistry.getPacket(tag.getStringOr("friendlyName", ""));
            }

            if (packetClass == null) {
                DihClientAddon.LOG.warn("[Dih] Rejected unknown/untrusted packet class: {}", className);
                return null;
            }
            ...
            if (!DihPacketRegistry.getC2SPackets().contains(packetClass)) {
                DihClientAddon.LOG.warn("[Dih] Deserialized packet {} is NOT a registered C2S packet! It may be skipped when sending.", className);
            }

            return packet;

        } catch (Exception e) {
            DihClientAddon.LOG.error("[Dih] Failed to deserialize packet", e);
            return null;
        }
    }
        ...
        while (clazz != null && clazz != Object.class) {
            for (Field field : clazz.getDeclaredFields()) {
                ...
                try {
                    ...
                    DihClientAddon.LOG.warn("[Dih] Failed to serialize field: {}", field.getName(), e);
                }
            }
        ...
        while (clazz != null && clazz != Object.class) {
            for (Field field : clazz.getDeclaredFields()) {
                ...
                try {
                    ...
                    DihClientAddon.LOG.warn("[Dih] Failed to deserialize field: {}", fieldName, e);
                }
            }
            clazz = clazz.getSuperclass();
        }
    ...
    private static boolean isKnownPacketClass(String className) {
        ...
        if (set == null) {
            ...
            try {
                for (Class<?> c : DihPacketRegistry.getC2SPackets()) built.add(c.getName());
                for (Class<?> c : DihPacketRegistry.getS2CPackets()) built.add(c.getName());
    ...
    private static Object getSafeDefault(Class<?> type, String fieldName) {
        ...
        if (type == CustomPacketPayload.class || CustomPacketPayload.class.isAssignableFrom(type)) {
            return new DihPayloadSupport.RawCustomPacketPayload(Identifier.fromNamespaceAndPath("minecraft", "empty"), new byte[0]);
        }
        ...
        try {
            try {
                ...
                if (Modifier.isStatic(emptyField.getModifiers())) {
                    ...
                    if (empty != null) {
                        DihClientAddon.LOG.warn("[Dih] Using {}.EMPTY for unsupported field: {}",
            ...
            try {
                ...
                if (Modifier.isStatic(defaultField.getModifiers())) {
                    ...
                    if (defaultValue != null) {
                        DihClientAddon.LOG.warn("[Dih] Using {}.DEFAULT for unsupported field: {}",
                            type.getSimpleName(), fieldName);
                        return defaultValue;
                    }
                }
            ...
            if (type.isRecord()) {
                ...
                for (int i = 0; i < components.length; i++) {
                    ...
                    if (paramTypes[i] == byte[].class) {
                        ...
                        try {
                            ...
                            DihClientAddon.LOG.warn("[Dih] Failed to create nested Record {}: {}",
                ...
                DihClientAddon.LOG.warn("[Dih] Created default {} for unsupported field: {}",
                    type.getSimpleName(), fieldName);
                return instance;
            }

        } catch (Exception e) {
            DihClientAddon.LOG.warn("[Dih] Could not create safe default for {}: {}",
                type.getSimpleName(), e.getMessage());
        }

        DihClientAddon.LOG.error("[Dih] No safe default found for unsupported type: {} field: {}",
            type.getSimpleName(), fieldName);
        return null;
    }
    ...
    public static String serializeQueueToBase64(List<DihSharedState.QueuedPacket> queue) {
        try {
            CompoundTag rootTag = new CompoundTag();
            ListTag packetList = new ListTag();
            ...
            DihClientAddon.LOG.error("[Dih] Failed to serialize queue to Base64", e);
            return null;
        }
    }

    public static List<DihSharedState.QueuedPacket> deserializeQueueFromBase64(String base64) {
        try {
            ...
            DihClientAddon.LOG.info("[Dih] Deserialized {} packets from Base64", queue.size());
            return queue.isEmpty() ? null : queue;
        } catch (Exception e) {
            DihClientAddon.LOG.error("[Dih] Failed to deserialize queue from Base64", e);
            return null;
        }
    }
}
