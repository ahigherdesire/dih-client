...
public final class AddonManager {
    ...
    private static final List<DihAddon> LOADED = new ArrayList<>();
    ...
    public static void init() {
        ...
        for (EntrypointContainer<DihAddon> container : FabricLoader.getInstance()
            ...
            try {
                ...
                if (declared <= 0) {
                    ...
                    DihClientAddon.LOG.warn("[Addons] Addon '{}' declared invalid API v{}; skipping it.", modId, declared);
                    continue;
                }
                if (declared > ApiVersion.CURRENT) {
                    report.markNeedsNewerApi("Needs API v" + declared + ", host provides v" + ApiVersion.CURRENT + ".");
                    DihClientAddon.LOG.warn("[Addons] Addon '{}' needs API v{} but this client provides v{}; skipping it.",
                ...
                DihClientAddon.LOG.error("[Addons] Addon '{}' does not declare apiVersion(); skipping it.", modId, e);
            } catch (Throwable t) {
                report.markFailed("prepare", "Failed to prepare: " + shortError(t));
                DihClientAddon.LOG.error("[Addons] Error preparing addon '{}'", modId, t);
            }
        }
        ...
        if (!LOADED.isEmpty()) {
            DihClientAddon.LOG.info("[Addons] Loaded {} addon(s).", LOADED.size());
        }
        if (config.disabledAddonIds != null) {
            for (String id : config.disabledAddonIds) {
                ...
                report.markDisabled("Disabled in DIH config, but no matching addon entrypoint was discovered.");
    ...
    private static boolean runPhase(LoadedAddon la, String phase, Runnable body) {
        ...
        try {
            ...
            DihClientAddon.LOG.error("[Addons] Addon '{}' threw during {}", la.modId, phase, t);
            return false;
        } finally {
            currentContext = null;
        }
    ...
    private static void invokeUnload(LoadedAddon la) {
        ...
        try {
            ...
            DihClientAddon.LOG.warn("[Addons] Addon '{}' threw during onUnload", la.modId, t);
        }
    }
    ...
    private static void rollbackRegistrations(String addonId) {
        ...
        rollbackStep(addonId, report, "macro actions", () -> dihclient.api.macro.MacroActionRegistry.unregisterAddon(addonId));
        rollbackStep(addonId, report, "custom menu adapters", () -> dihclient.api.custommenu.CustomMenuAdapterRegistry.unregisterAddon(addonId));
        rollbackStep(addonId, report, "presets", () -> dihclient.api.macro.MacroPresetRegistry.unregisterAddon(addonId));
        rollbackStep(addonId, report, "events", () -> dihclient.api.event.AddonEvents.unregisterAddon(addonId));
        rollbackStep(addonId, report, "hud", () -> dihclient.api.hud.HudElements.unregisterAddon(addonId));
        rollbackStep(addonId, report, "modules", () -> dihclient.modules.ModuleRegistry.unregisterAddonModules(addonId));
        rollbackStep(addonId, report, "commands", () -> dihclient.commands.DihCommands.unregisterAddonCommands(addonId));
    ...
    private static void rollbackStep(String addonId, AddonReport report, String kind, Runnable body) {
        try {
            ...
            DihClientAddon.LOG.warn("[Addons] Addon '{}' rollback failed for {}", addonId, kind, t);
        }
    }
    ...
    public static List<DihAddon> loaded() {
    ...
    public static void surfaceFailuresOnJoin() {
        ...
        DihNotifications.warning(issues + " addon issue(s). Open Addons for details.");
    }
    ...
    private record LoadedAddon(String modId, DihAddon addon) {}
    ...
    public static final class AddonReport {
        ...
        private void copyRuntimeMetadata(DihAddon addon) {
