...
public class SettingCommand extends Command {
    ...
    public void build(LiteralArgumentBuilder<AutismCommandSource> root) {
        ...
        root.then(RequiredArgumentBuilder.<AutismCommandSource, String>argument("module", ModuleArgumentType.moduleName())
            ...
            .then(RequiredArgumentBuilder.<AutismCommandSource, String>argument("key", StringArgumentType.word())
                ...
                .then(RequiredArgumentBuilder.<AutismCommandSource, String>argument("value", StringArgumentType.greedyString())
                    .executes(ctx -> set(
    ...
    private static int list(String moduleName) {
        ...
        if (settings.isEmpty()) {
            AutismClientMessaging.sendPrefixed("§7" + module.name() + " has no settings.");
            return SUCCESS;
        }
        AutismClientMessaging.sendPrefixed("§7Settings for §f" + module.name() + "§7:");
        for (Setting<?, ?> setting : settings) {
            AutismClientMessaging.sendPrefixed("§7- §f" + setting.id() + " §7= §b" + module.displayValue(setting));
        }
        return SUCCESS;
    }
    ...
    private static int get(String moduleName, String key) {
        ...
        AutismClientMessaging.sendPrefixed("§f" + module.name() + " §7" + key + " §7= §b" + module.value(key));
        return SUCCESS;
    }
    ...
    private static int set(String moduleName, String key, String value) {
        ...
        AutismClientMessaging.sendPrefixed("§a" + module.name() + " §7" + key + " §7-> §b" + module.value(key));
        return SUCCESS;
    }
    ...
    private static int notFound(String moduleName) {
        AutismClientMessaging.sendPrefixed("§cModule not found: §f" + moduleName);
        return SUCCESS;
    }

    private static int noKey(Module module, String key) {
        AutismClientMessaging.sendPrefixed("§c" + module.name() + " has no setting §f" + key);
        AutismClientMessaging.sendPrefixed("§7List them with §f" + AutismCommands.effectivePrefix() + "setting " + module.name().replace(' ', '-'));
        return SUCCESS;
    }
    ...
    private static int usage() {
        ...
        AutismClientMessaging.sendPrefixed("§f" + p + "setting <module> §7- list settings + values");
        AutismClientMessaging.sendPrefixed("§f" + p + "setting <module> <key> §7- show one value");
        AutismClientMessaging.sendPrefixed("§f" + p + "setting <module> <key> <value> §7- set it (value 'reset' = default)");
        return SUCCESS;
    }
}
